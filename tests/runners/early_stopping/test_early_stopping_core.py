...
def test_early_stopping_with_incomparable_vectors():
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        create_epoch_with_scores(
            work_dir, 1, {"loss1": 0.8, "loss2": 0.7}
        )  # incomparable (treated as no improvement)
        create_epoch_with_scores(
            work_dir, 2, {"loss1": 0.9, "loss2": 0.8}
        )  # incomparable (treated as no improvement)
...
def test_early_stopping_consecutive_epochs():
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        create_epoch_with_scores(
            work_dir, 3, {"loss": 0.6}
        )  # Should not be read due to missing epoch 2
...
def test_early_stopping_was_triggered_at_epoch():
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        create_epoch_with_scores(
            work_dir, 3, {"loss": 0.8}
        )  # no improvement (2nd) -> should trigger
        ...
        assert (
            early_stopping.was_triggered_at_epoch(2) == False
        )  # Only 1 epoch without improvement
        assert (
            early_stopping.was_triggered_at_epoch(3) == True
        )  # 2 epochs without improvement


@pytest.mark.parametrize(
    "direction,improving_scores,worsening_scores",
    [
        (-1, [1.0, 0.8, 0.6], [0.6, 0.8, 1.0]),  # Loss metric (lower better)
        (1, [0.6, 0.8, 1.0], [1.0, 0.8, 0.6]),  # Accuracy metric (higher better)
    ],
)
def test_early_stopping_direction_handling(
    direction, improving_scores, worsening_scores
):
