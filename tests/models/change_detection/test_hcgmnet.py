...
def test_hcgmnet_forward_pass(mode: str) -> None:
    ...
    if mode == 'train':
        ...
        assert outputs['intermediate_map'].shape == (
            4,
            2,
            224,
            224,
        ), f"{outputs['intermediate_map'].shape=}"
        assert outputs['change_map'].shape == (
            4,
            2,
            224,
            224,
        ), f"{outputs['change_map'].shape=}"
    else:
...
def test_hcgmnet_different_input_size(spatial_dim: int) -> None:
    ...
    ), "Model output should be a tensor in eval mode"
