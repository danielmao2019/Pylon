...
def test_feature_dimensions(test_batch):
    ...
    assert model.last_feature.shape == (
        num_points,
        64,
...
def test_feature_dimensions_step_by_step(test_batch):
    ...
    # the modules below take the batch index one number per point, as the model's own forward hands it to them
    pos1, batch1 = test_batch['pc_0'].xyz, test_batch['pc_0'].batch[:, 0]
    pos2, batch2 = test_batch['pc_1'].xyz, test_batch['pc_1'].batch[:, 0]
    ...
    assert (
        x1_down.shape[1] == 64
    ), f"Expected 64 channels after first down module, got {x1_down.shape[1]}"
    assert (
        x2_down.shape[1] == 64
    ), f"Expected 64 channels after first down module, got {x2_down.shape[1]}"
    ...
    assert (
        x1_down2.shape[1] == 128
    ), f"Expected 128 channels after second down module, got {x1_down2.shape[1]}"
    assert (
        x2_down2.shape[1] == 128
    ), f"Expected 128 channels after second down module, got {x2_down2.shape[1]}"
    ...
    assert (
        diff.shape[1] == 128
    ), f"Expected 128 channels after difference, got {diff.shape[1]}"
    ...
    assert (
        x.shape[1] == 192
    ), f"Expected 192 channels after concatenation (128 + 64), got {x.shape[1]}"
