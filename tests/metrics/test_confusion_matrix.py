...
@pytest.mark.parametrize(
    "y_pred, y_true, num_classes, expected",
    [
        (
            torch.tensor(
                [
            ...
            torch.tensor([7, 1, 4, 3], dtype=torch.int64),
            10,
            {
        ...
        (
            ...
            torch.tensor([7, 5, 4, 3], dtype=torch.int64),
            10,
            {
...
@pytest.mark.parametrize(
    "y_pred_list, y_true_list, num_classes, expected",
    [
        (
            [
                torch.tensor(
...
def test_confusion_matrix_summary(
    ...
    for key in expected:
        assert torch.equal(
            aggregated[key].isnan(), expected[key].isnan()
        ), f"{key=}, {aggregated[key]=}, {expected[key]=}, {aggregated[key].isnan()=}, {expected[key].isnan()=}"
        assert torch.equal(
            aggregated[key][~aggregated[key].isnan()],
            expected[key][~expected[key].isnan()],
        ), f"{key=}, {aggregated[key]=}, {expected[key]=}, {aggregated[key].isnan()=}, {expected[key].isnan()=}"
