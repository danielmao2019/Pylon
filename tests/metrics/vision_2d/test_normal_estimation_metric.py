...
def test_normal_estimation_metric_summarize(y_preds, y_trues):
    ...
    assert (
        result['per_datapoint']['angle'].ndim == 1
    )  # 1D tensor with one value per datapoint
    assert result['per_datapoint']['angle'].shape[0] == len(
        y_preds
    )  # Number of values matches number of datapoints
