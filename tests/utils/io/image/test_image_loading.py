...
import pytest
from utils.io.image import load_image, _load_image, _load_multispectral_image, _normalize
...
def test_load_image_with_sub_div_normalization():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert torch.allclose(result, torch.full((3, 50, 50), expected_value), atol=1e-5)
...
def test_load_image_invalid_inputs():
    ...
    with pytest.raises(ValueError, match="Exactly one of 'filepath' or 'filepaths' must be provided"):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(ValueError, match="'normalization' cannot be used together with 'sub' or 'div'"):
...
def test_normalize_multichannel():
    ...
    result = _normalize(tensor, sub=sub_per_channel, div=div_per_channel, normalization=None)
...
def test_normalize_invalid_inputs():
    ...
    with pytest.raises(ValueError, match="Normalization value must match the number of channels"):
        _normalize(tensor, sub=[0.1, 0.2], div=None, normalization=None)  # 2 values for 3 channels
