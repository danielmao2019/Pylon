...
from utils.io.point_cloud import (
    load_point_cloud, 
    _load_from_ply, 
    _load_from_txt, 
    _load_from_pth, 
...
def create_test_ply_file(filepath: str, include_rgb: bool = False, include_feat: bool = False):
    ...
    vertex_element = PlyElement.describe(
        np.array(vertices, dtype=dtype_list), 
        'vertex'
    )
...
def test_load_point_cloud_txt_with_features(temp_dir):
    ...
    assert result['feat'].dtype in [torch.float32, torch.float64]  # Allow both float types
...
def test_load_point_cloud_ply_missing_coordinates(temp_dir):
    ...
    try:
        vertex_element = PlyElement.describe(
            np.array(vertices, dtype=[('x', 'f4'), ('y', 'f4')]), 
            'vertex'
        )
        PlyData([vertex_element], text=True).write(filepath)
