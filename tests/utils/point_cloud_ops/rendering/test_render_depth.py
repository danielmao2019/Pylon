...
def test_render_depth_custom_ignore_value():
    ...
    pc_data = {'pos': torch.tensor([[0.0, 0.0, -1.0]], dtype=torch.float32)}
...
def test_render_depth_empty_pointcloud():
    ...
    with pytest.raises(
        AssertionError, match="Expected positive number of points, got 0"
    ):
...
def test_render_depth_invalid_inputs():
    ...
    valid_pc_data = {'pos': torch.tensor([[0.0, 0.0, -1.0]], dtype=torch.float32)}
