...
def test_render_rgb_depth_sorting():
    ...
    assert non_zero_pixels, f"No pixels were rendered. RGB max: {rgb_image.max()}, min: {rgb_image.min()}"
    ...
    if rgb_image.max() > 0:
        ...
        assert red_present or blue_present, "Neither red nor blue colors found in rendered image"
...
def test_render_rgb_invalid_inputs():
    ...
    with pytest.raises(AssertionError, match="convention must be 'opengl' or 'standard'"):
        render_rgb_from_pointcloud(
            pc_data=valid_pc_data,
            camera_intrinsics=valid_intrinsics,
            camera_extrinsics=valid_extrinsics,
