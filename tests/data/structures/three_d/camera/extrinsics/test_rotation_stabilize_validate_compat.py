...
def test_stabilize_accepts_float32_and_float64(dtype: torch.dtype) -> None:
    ...
    Args:
        dtype: The floating torch dtype under test.

    Returns:
        None.
    """
    r = _random_rotation(dtype=dtype, seed=1) @ _random_rotation(dtype=dtype, seed=2)
...
def test_stabilized_batch_passes_validator(dtype: torch.dtype) -> None:
    """A [B, 3, 3] batch stabilized in one call matches stabilizing each rotation alone, and the cam2world batch it builds passes validate_camera_extrinsics.
