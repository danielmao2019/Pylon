...
def valid_semseg_datapoint():
    ...
    return {
        ...
        'meta_info': {'image_path': '/path/to/image.jpg', 'index': 0},
    }
...
def valid_semseg_instance_datapoint():
    ...
    return {
        ...
        'labels': {
            'label': {
                'masks': [
                    torch.randint(0, 2, (64, 64), dtype=torch.bool) for _ in range(3)
                ],
                'indices': [1, 2, 3],
            }
        },
...
def valid_3dcd_datapoint():
    ...
    return {
        ...
        'labels': {'change_map': torch.randint(0, 2, (n_points,), dtype=torch.long)},
        'meta_info': {
...
def valid_pcr_batched_datapoint():
    ...
    return {
        ...
        'meta_info': {'batch_size': 2, 'index': 0},
    }
...
def test_validate_basic_datapoint_structure_missing_keys():
    ...
    # Missing 'labels' key
    invalid_datapoint = {'inputs': {'image': torch.randn(3, 32, 32)}, 'meta_info': {}}
...
def test_validate_semseg_structure_invalid_instance_format():
    ...
    assert "labels['label'] dict must have 'masks' and 'indices' keys" in str(
        exc_info.value
    )
...
def test_validate_2dcd_structure_missing_change_map():
    ...
    invalid_datapoint = {
        'inputs': {'img_1': torch.randn(3, 32, 32), 'img_2': torch.randn(3, 32, 32)},
...
def test_validate_3dcd_structure_empty_pc():
    ...
    invalid_datapoint = {
        'inputs': {'pc_1': "not_a_pointcloud", 'pc_2': pc_2},
        'labels': {'change_map': torch.randint(0, 2, (100,))},
        'meta_info': {},
    }
...
def test_validate_pcr_structure_batched_missing_lengths():
    ...
    assert "inputs must have 'src_pc' key" in str(
        exc_info.value
    )  # Falls back to single format validation
...
@pytest.mark.parametrize(
    "dataset_type,datapoint_fixture",
    [
...
def test_validate_structure_for_type_invalid_type():
    ...
    assert "Unsupported dataset type for validation: invalid_type" in str(
        exc_info.value
    )
...
@pytest.mark.parametrize(
    "dataset_type,invalid_datapoint",
    [
        (
            'semseg',
            {
                'inputs': {'wrong_key': torch.randn(3, 32, 32)},
                'labels': {'label': torch.randint(0, 10, (32, 32))},
                'meta_info': {},
            },
        ),
        ...
        (
            '3dcd',
            {
                'inputs': {'pc_1': PointCloud(xyz=torch.randn(100, 3))},
                'labels': {'change_map': torch.randint(0, 2, (100,))},
                'meta_info': {},
            },
        ),
        (
            'pcr',
            {
                'inputs': {'src_pc': PointCloud(xyz=torch.randn(100, 3))},
                'labels': {'transform': torch.eye(4)},
                'meta_info': {},
            },
        ),
