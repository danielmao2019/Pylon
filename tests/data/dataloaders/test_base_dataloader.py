...
@pytest.mark.parametrize(
    "dataset_size, batch_size, last_mode, expected_num_batches, expected_last_batch_size",
    [
...
def test_base_dataloader(
    ...
    if batches:
        ...
        assert (
            len(last_batch) == expected_last_batch_size
        ), f"Expected last batch size {expected_last_batch_size}, got {len(last_batch)}"
...
def test_dataloader_shuffle_behavior(shuffle):
    ...
    if shuffle:
        assert not all(
            torch.equal(b1, b2) for b1, b2 in zip(batches1, batches2, strict=True)
        ), "Shuffled batches should not be identical"
    else:
        assert all(
            torch.equal(b1, b2) for b1, b2 in zip(batches1, batches2, strict=True)
        ), "Non-shuffled batches should be identical"
...
def test_dataloader_with_empty_dataset():
    ...
    assert (
        len(list(dataloader)) == 0
    ), "Dataloader should yield no batches for an empty dataset"
