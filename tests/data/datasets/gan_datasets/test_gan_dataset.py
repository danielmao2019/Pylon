...
def validate_inputs(inputs: Dict[str, Any], dataset) -> None:
    ...
    assert (
        inputs['z'].device.type == dataset.device.type
    ), f"{inputs['z'].device=}, {dataset.device=}"
...
def validate_labels(labels: Dict[str, Any], dataset) -> None:
    ...
    assert (
        labels['image'].device.type == dataset.device.type
    ), f"{labels['image'].device=}, {dataset.device=}"
...
def test_reproducibility(gan_dataset_config, max_samples, get_samples_to_test):
    ...
    def test_reproducibility_for_idx(idx: int) -> None:
        ...
        assert torch.allclose(
            inputs1['z'], inputs2['z']
        ), f"Latent vector not reproducible at idx {idx}"
