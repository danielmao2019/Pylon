...
def test_gan_dataset_version_discrimination():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        source_dataset = MNISTDataset(data_root=temp_dir, split='train')

        # Same parameters should have same hash
        dataset1a = GANDataset(source=source_dataset, latent_dim=100)
        dataset1b = GANDataset(source=source_dataset, latent_dim=100)
        assert dataset1a.get_cache_version_hash() == dataset1b.get_cache_version_hash()

        # Different latent_dim should have different hash
        dataset2 = GANDataset(source=source_dataset, latent_dim=200)  # Different
        assert dataset1a.get_cache_version_hash() != dataset2.get_cache_version_hash()

        # Different source dataset should have different hash
        source_dataset2 = MNISTDataset(
            data_root=temp_dir, split='test'  # Different split
        )
        dataset3 = GANDataset(
            source=source_dataset2, latent_dim=100  # Different source
        )
...
def test_latent_dim_variants():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for latent_dim in latent_dim_variants:
            dataset = GANDataset(source=source_dataset, latent_dim=latent_dim)
...
def test_inherited_parameters_affect_version_hash():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset2 = GANDataset(
            source=source_dataset, latent_dim=100, dataset_size=50
        )  # Different size
