...
def test_synth_pcr_dataset(dataset_params):
    ...
    for idx in range(len(dataset)):
        ...
        assert torch.allclose(R @ R.T, torch.eye(3), atol=1e-6), "Invalid rotation matrix: not orthogonal"
        assert torch.abs(torch.det(R) - 1.0) < 1e-6, "Invalid rotation matrix: determinant not 1"
        
        # Check rotation magnitude
        rot_angle = torch.acos(torch.clamp((torch.trace(R) - 1) / 2, -1, 1))
        assert torch.abs(rot_angle) <= np.radians(dataset_params['rot_mag']), "Rotation angle exceeds specified limit"
        
        # Check translation magnitude
        assert torch.norm(t) <= dataset_params['trans_mag'], "Translation magnitude exceeds specified limit"
