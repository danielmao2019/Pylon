...
from data.datasets.change_detection_datasets.single_temporal.bi2single_temporal_dataset import (
    Bi2SingleTemporal,
)
...
def test_bi2single_temporal_different_source_different_hash(levir_cd_data_root):
    ...
    ), f"Different source datasets should produce different hashes: {hash_train} == {hash_test}"


def test_bi2single_temporal_different_source_data_root_different_hash(
    levir_cd_data_root,
):
    ...
    assert (
        hash1 != hash2
    ), f"Different source splits should produce different hashes: {hash1} == {hash2}"
...
def test_bi2single_temporal_comprehensive_no_hash_collisions(levir_cd_data_root):
    ...
    for source in sources:
        ...
        ), f"Hash collision detected for source split {source.split}: hash {hash_val} already exists"
        hashes.append(hash_val)

    # Verify we generated the expected number of unique hashes
    assert len(hashes) == len(
        sources
    ), f"Expected {len(sources)} unique hashes, got {len(hashes)}"

    # Verify we have exactly 2 unique hashes (train and test)
    assert (
        len(set(hashes)) == 2
    ), f"Expected 2 unique hashes for train/test splits, got {len(set(hashes))}: {hashes}"
