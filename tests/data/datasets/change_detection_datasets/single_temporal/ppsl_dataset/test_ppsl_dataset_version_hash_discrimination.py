...
def test_ppsl_different_source_different_hash():
    ...
    source2 = SemanticSegmentationRandomDataset(
        num_examples=20, num_classes=5, base_seed=42  # Different dataset size
    )
...
def test_ppsl_different_source_seed_different_hash():
    ...
    source2 = SemanticSegmentationRandomDataset(
        num_examples=10, num_classes=5, base_seed=123  # Different seed
    )
...
def test_ppsl_different_source_num_classes_different_hash():
    ...
    source2 = SemanticSegmentationRandomDataset(
        num_examples=10, num_classes=10, base_seed=42  # Different number of classes
    )
    ...
    assert (
        hash1 != hash2
    ), f"Different source num_classes should produce different hashes: {hash1} == {hash2}"
...
def test_ppsl_comprehensive_no_hash_collisions():
    ...
    for i, source_config in enumerate(source_configs):
        source = SemanticSegmentationRandomDataset(**source_config)
        ...
        ), f"Hash collision detected for config {i}: {source_config}, hash {hash_val} already exists"
        hashes.append(hash_val)

    # Verify we generated the expected number of unique hashes
    assert len(hashes) == len(
        source_configs
    ), f"Expected {len(source_configs)} unique hashes, got {len(hashes)}"
