...
def test_kc3d_same_parameters_same_hash(create_dummy_kc3d_files):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset1a = KC3DDataset(
            data_root=temp_dir, split='train', use_ground_truth_registration=True
        )
        dataset1b = KC3DDataset(
            data_root=temp_dir, split='train', use_ground_truth_registration=True
        )
...
def test_kc3d_different_split_different_hash(create_dummy_kc3d_files):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset_train = KC3DDataset(
            data_root=temp_dir, split='train', use_ground_truth_registration=True
        )
        dataset_val = KC3DDataset(
            data_root=temp_dir, split='val', use_ground_truth_registration=True
        )
        dataset_test = KC3DDataset(
            data_root=temp_dir, split='test', use_ground_truth_registration=True
        )
...
def test_kc3d_different_use_ground_truth_registration_different_hash(
    create_dummy_kc3d_files,
):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset_with_gt = KC3DDataset(
            data_root=temp_dir, split='train', use_ground_truth_registration=True
        )
        dataset_without_gt = KC3DDataset(
            data_root=temp_dir, split='train', use_ground_truth_registration=False
        )
        ...
        assert (
            hash_with_gt != hash_without_gt
        ), f"Different use_ground_truth_registration should produce different hashes: {hash_with_gt} == {hash_without_gt}"
...
def test_kc3d_different_data_root_different_hash(create_dummy_kc3d_files):
    ...
    with tempfile.TemporaryDirectory() as temp_dir1:
        with tempfile.TemporaryDirectory() as temp_dir2:
            ...
            assert (
                hash1 == hash2
            ), f"Different data roots should produce SAME hashes (data_root excluded): {hash1} != {hash2}"
