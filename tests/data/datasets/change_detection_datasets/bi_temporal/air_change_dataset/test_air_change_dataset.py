...
def validate_inputs(inputs: Dict[str, Any], dataset: AirChangeDataset) -> None:
    ...
    assert set(inputs.keys()) == set(
        dataset.INPUT_NAMES
    ), f"Unexpected input keys: {inputs.keys()}"

    for img_name in ["img_1", "img_2"]:
        img = inputs[img_name]
        assert isinstance(
            img, torch.Tensor
        ), f"{img_name} should be a torch.Tensor, got {type(img)}"
        assert img.ndim == 3, f"{img_name} should have 3 dimensions, got {img.ndim}"
        assert (
            img.size(0) == 3
        ), f"{img_name} should have 3 channels (C x H x W), got {img.size(0)}"
        assert (
            img.dtype == torch.float32
        ), f"{img_name} should be of dtype torch.float32, got {img.dtype}"


def validate_labels(
    labels: Dict[str, Any], class_dist: torch.Tensor, dataset: AirChangeDataset
) -> None:
    ...
    assert set(labels.keys()) == set(
        dataset.LABEL_NAMES
    ), f"Unexpected label keys: {labels.keys()}"

    change_map = labels['change_map']
    assert isinstance(
        change_map, torch.Tensor
    ), f"Expected change_map to be a torch.Tensor, got {type(change_map)}"
...
def validate_class_distribution(
    class_dist: torch.Tensor, dataset: AirChangeDataset, num_samples: int
) -> None:
    ...
    if num_samples == len(dataset):
        assert isinstance(
            dataset.CLASS_DIST, list
        ), f"CLASS_DIST should be a list, got {type(dataset.CLASS_DIST)}"
        if dataset.split == 'train':
            assert (
                abs(
                    class_dist[1] / class_dist[0]
                    - dataset.CLASS_DIST[1] / dataset.CLASS_DIST[0]
                )
                < 1.0e-02
            )
        else:
            ...
            ), f"Class distribution mismatch: {class_dist=}, {dataset.CLASS_DIST=}"
...
def test_air_change(dataset_config, max_samples, get_samples_to_test) -> None:
    ...
    def validate_datapoint(idx: int) -> None:
        ...
        assert isinstance(
            datapoint, dict
        ), f"Expected datapoint to be a dict, got {type(datapoint)}"
        assert set(datapoint.keys()) == {
            "inputs",
            "labels",
            "meta_info",
        }, f"Unexpected keys in datapoint: {datapoint.keys()}"
