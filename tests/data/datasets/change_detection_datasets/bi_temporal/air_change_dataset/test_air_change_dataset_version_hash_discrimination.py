...
def test_air_change_same_parameters_same_hash(create_dummy_air_change_files):
    """Test that identical parameters produce identical hashes."""
    
    with tempfile.TemporaryDirectory() as temp_dir:
        create_dummy_air_change_files(temp_dir)
        
        # Same parameters should produce same hash
        dataset1a = AirChangeDataset(data_root=temp_dir, split='train')
        dataset1b = AirChangeDataset(data_root=temp_dir, split='train')
...
def test_air_change_different_split_different_hash(create_dummy_air_change_files):
    """Test that different splits produce different hashes."""
    
    with tempfile.TemporaryDirectory() as temp_dir:
        create_dummy_air_change_files(temp_dir)
        
        dataset_train = AirChangeDataset(data_root=temp_dir, split='train')
        dataset_test = AirChangeDataset(data_root=temp_dir, split='test')
...
def test_air_change_different_data_root_different_hash(create_dummy_air_change_files):
    """Test that different data roots produce different hashes."""
    
    with tempfile.TemporaryDirectory() as temp_dir1:
        with tempfile.TemporaryDirectory() as temp_dir2:
            create_dummy_air_change_files(temp_dir1)
            create_dummy_air_change_files(temp_dir2)
            
            dataset1 = AirChangeDataset(data_root=temp_dir1, split='train')
            dataset2 = AirChangeDataset(data_root=temp_dir2, split='train')
...
def test_air_change_hash_format(create_dummy_air_change_files):
    """Test that hash is in correct format."""
    
    with tempfile.TemporaryDirectory() as temp_dir:
        create_dummy_air_change_files(temp_dir)
        
        dataset = AirChangeDataset(data_root=temp_dir, split='train')
...
def test_air_change_comprehensive_no_hash_collisions(create_dummy_air_change_files):
