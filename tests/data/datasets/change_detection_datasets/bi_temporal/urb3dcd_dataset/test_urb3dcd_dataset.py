...
def validate_point_cloud(pc: PointCloud, name: str) -> None:
    ...
    # the version's element is loaded at whatever width it stores, and the dataset narrows to the width its models train at
    assert (
        pc.xyz.dtype == torch.float32
    ), f"{name}.xyz is not the width the dataset narrows to: {pc.xyz.dtype=}"
