...
def test_slpccd_same_parameters_same_hash(slpccd_data_root):
    ...
    dataset1a = SLPCCDDataset(
        data_root=slpccd_data_root,
        split='train',
        num_points=8192,
        random_subsample=True,
    )
    dataset1b = SLPCCDDataset(
        data_root=slpccd_data_root,
        split='train',
        num_points=8192,
        random_subsample=True,
...
def test_slpccd_different_num_points_different_hash(slpccd_data_root):
    ...
    dataset_4096 = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', num_points=4096
    )
    dataset_8192 = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', num_points=8192
    )
    ...
    assert (
        hash_4096 != hash_8192
    ), f"Different num_points should produce different hashes: {hash_4096} == {hash_8192}"
...
def test_slpccd_different_random_subsample_different_hash(slpccd_data_root):
    ...
    dataset_random = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', random_subsample=True
    )
    dataset_no_random = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', random_subsample=False
    )
    ...
    assert (
        hash_random != hash_no_random
    ), f"Different random_subsample should produce different hashes: {hash_random} == {hash_no_random}"
...
def test_slpccd_different_use_hierarchy_different_hash(slpccd_data_root):
    ...
    dataset_hierarchy = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', use_hierarchy=True
    )
    dataset_no_hierarchy = SLPCCDDataset(
    ...
    assert (
        hash_hierarchy != hash_no_hierarchy
    ), f"Different use_hierarchy should produce different hashes: {hash_hierarchy} == {hash_no_hierarchy}"
...
def test_slpccd_different_hierarchy_levels_different_hash(slpccd_data_root):
    ...
    dataset_2_levels = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', hierarchy_levels=2
    )
    dataset_3_levels = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', hierarchy_levels=3
    )
    ...
    assert (
        hash_2_levels != hash_3_levels
    ), f"Different hierarchy_levels should produce different hashes: {hash_2_levels} == {hash_3_levels}"
...
def test_slpccd_different_knn_size_different_hash(slpccd_data_root):
    ...
    dataset_knn_16 = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', knn_size=16
    )
    ...
    assert (
        hash_knn_8 != hash_knn_16
    ), f"Different knn_size should produce different hashes: {hash_knn_8} == {hash_knn_16}"
...
def test_slpccd_different_cross_knn_size_different_hash(slpccd_data_root):
    ...
    dataset_cross_8 = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', cross_knn_size=8
    )
    dataset_cross_16 = SLPCCDDataset(
        data_root=slpccd_data_root, split='train', cross_knn_size=16
    )
    ...
    assert (
        hash_cross_8 != hash_cross_16
    ), f"Different cross_knn_size should produce different hashes: {hash_cross_8} == {hash_cross_16}"
...
def test_slpccd_different_data_root_different_hash(slpccd_data_root):
    ...
    assert (
        hash1 != hash2
    ), f"Different splits should produce different hashes: {hash1} == {hash2}"
