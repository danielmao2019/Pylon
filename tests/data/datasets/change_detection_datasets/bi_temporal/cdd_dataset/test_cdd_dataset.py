...
def validate_inputs(inputs: Dict[str, Any], dataset: CDDDataset, idx: int) -> None:
    ...
    assert (
        img_1.ndim == 3 and img_2.ndim == 3
    ), f"Input images at index {idx} must be 3D tensors."
    assert (
        img_1.dtype == torch.float32 and img_2.dtype == torch.float32
...
def validate_labels(
    labels: Dict[str, Any], class_dist: torch.Tensor, dataset: CDDDataset, idx: int
) -> None:
...
def validate_meta_info(meta_info: Dict[str, Any], datapoint_idx: int) -> None:
    ...
    assert isinstance(
        meta_info, dict
    ), f"Meta info at index {datapoint_idx} is not a dictionary."
    assert (
        'idx' in meta_info
    ...
    assert (
        'image_resolution' in meta_info
    ), f"Missing 'image_resolution' in meta info at index {datapoint_idx}."


def validate_class_distribution(
    class_dist: torch.Tensor, dataset: CDDDataset, num_samples: int
) -> None:
...
def test_cdd_dataset(dataset_config, max_samples, get_samples_to_test) -> None:
    ...
    ), "Dataset is not a valid PyTorch dataset instance."
    ...
    def validate_datapoint(idx: int) -> None:
        ...
        assert isinstance(
            datapoint, dict
        ), f"Datapoint at index {idx} is not a dictionary."
