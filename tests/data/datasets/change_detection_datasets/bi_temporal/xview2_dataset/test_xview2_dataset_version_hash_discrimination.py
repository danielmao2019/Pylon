...
def test_xview2_different_split_different_hash(xview2_data_root):
    ...
    assert (
        hash_train != hash_hold
    ), f"Different splits should produce different hashes: {hash_train} == {hash_hold}"
    assert (
        hash_test != hash_hold
    ), f"Different splits should produce different hashes: {hash_test} == {hash_hold}"
