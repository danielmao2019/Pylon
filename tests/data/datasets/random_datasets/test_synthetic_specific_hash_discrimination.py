...
@pytest.mark.parametrize(
    'random_dataset_class,base_args',
    [
...
def test_synthetic_num_examples_discrimination(random_dataset_class, base_args):
    ...
    config1 = {'class': dataset_class, 'args': {**base_args, 'num_examples': 50}}
    config2 = {'class': dataset_class, 'args': {**base_args, 'num_examples': 100}}
    ...
    assert (
        hash1 != hash2
    ), f"{random_dataset_class}: Different num_examples should produce different hashes: {hash1} == {hash2}"
...
def test_synthetic_num_classes_discrimination(random_dataset_class, base_args):
    ...
    assert (
        hash1 != hash2
    ), f"{random_dataset_class}: Different num_classes should produce different hashes: {hash1} == {hash2}"
...
def test_classification_random_image_res_discrimination():
    ...
    assert (
        hash1 != hash2
    ), f"ClassificationRandomDataset: Different image_res should produce different hashes: {hash1} == {hash2}"
...
def test_semantic_segmentation_random_initial_seed_discrimination():
    ...
    assert (
        hash1 != hash2
    ), f"SemanticSegmentationRandomDataset: Different initial_seed should produce different hashes: {hash1} == {hash2}"
...
def test_synthetic_comprehensive_parameter_combinations():
    ...
    for combination in test_combinations:
        config = {'class': ClassificationRandomDataset, 'args': combination}
        ...
        ), f"Hash collision detected for {combination}: hash {hash_val} already exists"
        hashes.append(hash_val)

    # Verify we have unique hashes
    assert len(hashes) == len(
        test_combinations
    ), f"Expected {len(test_combinations)} unique hashes, got {len(hashes)}"
