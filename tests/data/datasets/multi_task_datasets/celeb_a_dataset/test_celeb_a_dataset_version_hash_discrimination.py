...
def test_celeb_a_same_parameters_same_hash(create_dummy_celeb_a_files):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset1a = CelebADataset(data_root=temp_dir, split='train', use_landmarks=True)
        dataset1b = CelebADataset(data_root=temp_dir, split='train', use_landmarks=True)
...
def test_celeb_a_different_use_landmarks_different_hash(create_dummy_celeb_a_files):
    """Test that different use_landmarks settings produce different hashes."""
    
    with tempfile.TemporaryDirectory() as temp_dir:
        create_dummy_celeb_a_files(temp_dir)
        
        dataset_with_landmarks = CelebADataset(data_root=temp_dir, split='train', use_landmarks=True)
        dataset_without_landmarks = CelebADataset(data_root=temp_dir, split='train', use_landmarks=False)
...
def test_celeb_a_different_split_different_hash(create_dummy_celeb_a_files):
    """Test that different splits produce different hashes."""
    
    with tempfile.TemporaryDirectory() as temp_dir:
        create_dummy_celeb_a_files(temp_dir)
        
        dataset_train = CelebADataset(data_root=temp_dir, split='train', use_landmarks=False)
        dataset_val = CelebADataset(data_root=temp_dir, split='val', use_landmarks=False)
...
def test_celeb_a_hash_format(create_dummy_celeb_a_files):
...
def test_celeb_a_comprehensive_no_hash_collisions(create_dummy_celeb_a_files):
    """Test that different configurations produce unique hashes (no collisions)."""
    
    with tempfile.TemporaryDirectory() as temp_dir1:
        with tempfile.TemporaryDirectory() as temp_dir2:
            create_dummy_celeb_a_files(temp_dir1)
            create_dummy_celeb_a_files(temp_dir2)
            
            # Test various parameter combinations
            configs = [
                {'data_root': temp_dir1, 'split': 'train', 'use_landmarks': True},
                {'data_root': temp_dir1, 'split': 'train', 'use_landmarks': False},
                {'data_root': temp_dir1, 'split': 'val', 'use_landmarks': True},
                {'data_root': temp_dir1, 'split': 'val', 'use_landmarks': False},
                {'data_root': temp_dir2, 'split': 'train', 'use_landmarks': True},
                {'data_root': temp_dir2, 'split': 'train', 'use_landmarks': False},
            ]
