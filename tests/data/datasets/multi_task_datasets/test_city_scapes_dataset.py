...
def validate_inputs(inputs: Dict[str, Any], image_resolution: tuple) -> None:
    ...
    assert (
        image.shape[-2:] == image_resolution
    ), f"{image.shape[-2:]=}, {image_resolution=}"


def validate_labels(
    labels: Dict[str, Any], dataset: CityScapesDataset, image_resolution: tuple
) -> None:
    assert isinstance(labels, dict), f"{type(labels)=}"
    # Check that all label keys are valid CityScapes labels
    assert set(labels.keys()).issubset(
        set(CityScapesDataset.LABEL_NAMES)
    ), f"Invalid label keys: {set(labels.keys()) - set(CityScapesDataset.LABEL_NAMES)}"
    ...
    if 'semantic_segmentation' in labels:
        ...
        assert set(semantic_segmentation.unique().tolist()).issubset(
            set(list(dataset.CLASS_MAP.values()) + [dataset.IGNORE_INDEX])
        )
    ...
    if 'instance_segmentation' in labels:
        ...
        ), f"{type(instance_segmentation)=}"
        assert (
            instance_segmentation.ndim == 3 and instance_segmentation.shape[0] == 2
        ), f"{instance_segmentation.shape=}"
        assert (
            instance_segmentation.dtype == torch.float32
        ), f"{instance_segmentation.dtype=}"
        assert (
            instance_segmentation.shape[-2:] == image_resolution
        ), f"{instance_segmentation.shape=}, {image_resolution=}"


def validate_meta_info(
    meta_info: Dict[str, Any], datapoint_idx: int, dataset: CityScapesDataset
) -> None:
...
def test_city_scapes(
    city_scapes_dataset_config, max_samples, get_samples_to_test
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ['depth_estimation', 'instance_segmentation'],
            ['depth_estimation', 'instance_segmentation'],
        ),
        (None, None),  # Default case - will be set to CityScapesDataset.LABEL_NAMES
    ],
)
def test_city_scapes_selective_loading(
    city_scapes_base_config, selected_labels, expected_keys
):
