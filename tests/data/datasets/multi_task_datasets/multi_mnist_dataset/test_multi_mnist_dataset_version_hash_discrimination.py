...
def test_multi_mnist_dataset_version_discrimination():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset1a = MultiMNISTDataset(data_root=temp_dir, split='train')
        dataset1b = MultiMNISTDataset(data_root=temp_dir, split='train')
        assert dataset1a.get_cache_version_hash() == dataset1b.get_cache_version_hash()

        # Different split should have different hash
        dataset2 = MultiMNISTDataset(data_root=temp_dir, split='val')  # Different
...
def test_split_variants():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for split in split_variants:
            dataset = MultiMNISTDataset(data_root=temp_dir, split=split)
