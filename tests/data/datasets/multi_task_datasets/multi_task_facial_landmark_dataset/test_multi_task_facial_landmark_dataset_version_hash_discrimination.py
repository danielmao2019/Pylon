...
def create_dummy_facial_landmark_structure(data_root: str) -> None:
    ...
    with open(os.path.join(data_root, 'training.txt'), 'w') as f:
        for (
            ...
            line_parts = (
                [img_path]
                + [str(x) for x in landmarks_x]
                + [str(y) for y in landmarks_y]
                + [str(gender), str(smile), str(glasses), str(pose)]
            )
...
def test_multi_task_facial_landmark_dataset_version_discrimination():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset1a = MultiTaskFacialLandmarkDataset(data_root=temp_dir, split='train')
        dataset1b = MultiTaskFacialLandmarkDataset(data_root=temp_dir, split='train')
        assert dataset1a.get_cache_version_hash() == dataset1b.get_cache_version_hash()

        # Different split should have different hash
        dataset2 = MultiTaskFacialLandmarkDataset(
            data_root=temp_dir, split='test'  # Different
        )
...
def test_split_variants():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for split in split_variants:
            dataset = MultiTaskFacialLandmarkDataset(data_root=temp_dir, split=split)
...
def test_different_selective_loading_configurations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset1 = MultiTaskFacialLandmarkDataset(
            data_root=temp_dir, split='train', labels=['landmarks', 'gender']
        )

        dataset2 = MultiTaskFacialLandmarkDataset(
            data_root=temp_dir, split='train', labels=['smile', 'glasses', 'pose']
        )

        dataset3 = MultiTaskFacialLandmarkDataset(
            data_root=temp_dir, split='train'  # No labels parameter (loads all)
        )
...
def test_comprehensive_no_hash_collisions():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for split in ['train', 'test']:
            ...
            datasets.append(
                MultiTaskFacialLandmarkDataset(data_root=temp_dir, split=split)
            )

            # Test different selective loading configurations
            datasets.append(
                MultiTaskFacialLandmarkDataset(
                    data_root=temp_dir, split=split, labels=['landmarks']
                )
            )

            datasets.append(
                MultiTaskFacialLandmarkDataset(
                    data_root=temp_dir, split=split, labels=['gender', 'smile']
                )
            )
