...
def validate_inputs(inputs: Dict[str, Any]) -> None:
    ...
    assert (
        -1 <= inputs['image'].min() <= inputs['image'].max() <= +1
    ), f"{inputs['image'].min()=}, {inputs['image'].max()=}"


def validate_labels(
    labels: Dict[str, Any], dataset: NYUv2Dataset, image_resolution: tuple
) -> None:
    assert isinstance(labels, dict), f"{type(labels)=}"
    # Check that all label keys are valid NYUv2 labels
    assert set(labels.keys()).issubset(
        set(NYUv2Dataset.LABEL_NAMES)
    ), f"Invalid label keys: {set(labels.keys()) - set(NYUv2Dataset.LABEL_NAMES)}"
    ...
    if 'edge_detection' in labels:
        ...
        assert (
            edge_detection.ndim == 3 and edge_detection.shape[0] == 1
        ), f"{edge_detection.shape=}"
        assert edge_detection.dtype == torch.float32, f"{edge_detection.dtype=}"
        assert set(edge_detection.unique().tolist()) == set(
            [0, 1]
        ), f"{edge_detection.unique().tolist()=}"
        assert (
            edge_detection.shape[-2:] == image_resolution
        ), f"{edge_detection.shape=}, {image_resolution=}"
    ...
    if 'depth_estimation' in labels:
        ...
        assert (
            depth_estimation.shape == image_resolution
        ), f"{depth_estimation.shape=}, {image_resolution=}"

    if 'normal_estimation' in labels:
        normal_estimation = labels['normal_estimation']
        assert isinstance(
            normal_estimation, torch.Tensor
        ), f"{type(normal_estimation)=}"
        assert (
            normal_estimation.ndim == 3 and normal_estimation.shape[0] == 3
        ), f"{normal_estimation.shape=}"
        assert normal_estimation.dtype == torch.float32, f"{normal_estimation.dtype=}"
        assert (
            normal_estimation.shape[-2:] == image_resolution
        ), f"{normal_estimation.shape=}, {image_resolution=}"

    if 'semantic_segmentation' in labels:
        semantic_segmentation = labels['semantic_segmentation']
        assert isinstance(
            semantic_segmentation, torch.Tensor
        ), f"{type(semantic_segmentation)=}"
        assert semantic_segmentation.ndim == 2, f"{semantic_segmentation.shape=}"
        assert (
            semantic_segmentation.dtype == torch.int64
        ), f"{semantic_segmentation.dtype=}"
        assert (
            semantic_segmentation.shape == image_resolution
        ), f"{semantic_segmentation.shape=}, {image_resolution=}"
        assert set(semantic_segmentation.unique().tolist()).issubset(
            set(range(dataset.NUM_CLASSES))
        )


def validate_meta_info(
    meta_info: Dict[str, Any], datapoint_idx: int, dataset: NYUv2Dataset
) -> None:
    ...
    assert os.path.isfile(
        os.path.join(dataset.data_root, image_filepath)
    ), f"File does not exist: {os.path.join(dataset.data_root, image_filepath)}"
...
def validate_datapoint(dataset: NYUv2Dataset, idx: int) -> None:
    ...
    validate_labels(
        datapoint['labels'], dataset, datapoint['meta_info']['image_resolution']
    )
    validate_meta_info(datapoint['meta_info'], idx, dataset)
    # Additional validation for image resolution consistency
    assert (
        datapoint['inputs']['image'].shape[-2:]
        == datapoint['meta_info']['image_resolution']
    )


@pytest.mark.parametrize(
    'nyu_v2_dataset_config',
    [
        {
            'split': 'train',
        },
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ['depth_estimation', 'normal_estimation'],
            ['depth_estimation', 'normal_estimation'],
        ),
        (
            ['semantic_segmentation', 'edge_detection'],
            ['semantic_segmentation', 'edge_detection'],
        ),
        (
            ['depth_estimation', 'semantic_segmentation', 'edge_detection'],
            ['depth_estimation', 'semantic_segmentation', 'edge_detection'],
        ),
