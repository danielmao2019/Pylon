...
@pytest.fixture
def basic_datapoint():
    return {'inputs': {'x': 0, 'y': 1}, 'labels': {'a': 2, 'b': 3}, 'meta_info': {}}


@pytest.mark.parametrize(
    "transforms, expected_parsed",
    [
...
def test_compose_init(transforms, expected_parsed):
    ...
    assert len(compose.transforms) == len(
        expected_parsed
    ...
    for actual, expected in zip(compose.transforms, expected_parsed, strict=True):
        ...
        assert (
            actual["op"].__code__.co_code == expected["op"].__code__.co_code
        ), "Transform function mismatch"

        # Check input names
        assert (
            actual["input_names"] == expected["input_names"]
        ), f"Input names mismatch: {actual['input_names']} != {expected['input_names']}"

        # Check output names
        assert (
            actual["output_names"] == expected["output_names"]
        ), f"Output names mismatch: {actual['output_names']} != {expected['output_names']}"


@pytest.mark.parametrize(
    "transforms, example, expected",
    [
...
@pytest.mark.parametrize(
    "transforms, example, error_type, error_msg",
    [
