...
@pytest.fixture
def combined_cache():
    """Create a temporary combined cache for testing."""
    with tempfile.TemporaryDirectory() as temp_dir:
        cache = CombinedDatasetCache(
            ...
            use_cpu_cache=True,
            use_disk_cache=True,
            max_cpu_memory_percent=10.0,  # Small cache for testing
...
@pytest.fixture
def combined_cache_with_items(combined_cache, sample_datapoint):
    """Create a combined cache pre-populated with 3 items."""
    for i in range(3):
        combined_cache.put(i, copy.deepcopy(sample_datapoint))
