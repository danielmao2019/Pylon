...
def test_lru_eviction_scenarios(
    ...
    for item in expected_evicted:
        assert (
            cache_key_factory(item) not in cache.cache
        ), f"Item {item} should have been evicted"

    # Verify retained items
    for item in expected_retained:
        assert (
            cache_key_factory(item) in cache.cache
        ), f"Item {item} should have been retained"
