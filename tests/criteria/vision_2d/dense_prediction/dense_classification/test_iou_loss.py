...
def test_iou_loss_basic(sample_data):
    """
    Test basic functionality of IoULoss.
    """
    y_pred, y_true = sample_data
    device = y_pred.device

    # Initialize criterion
    criterion = IoULoss(num_classes=y_pred.size(1)).to(device)
...
def test_iou_loss_perfect_predictions(sample_data):
    """
    Test IoULoss with perfect predictions.
    """
    y_pred, y_true = sample_data
    device = y_pred.device
...
def test_iou_loss_with_class_weights(sample_data):
    """
    Test IoULoss with class weights.
    """
    y_pred, y_true = sample_data
    device = y_pred.device
    num_classes = y_pred.size(1)
    ...
    criterion = IoULoss(num_classes=num_classes, class_weights=class_weights).to(device)

    # Compute loss with weights
    loss_weighted = criterion(y_pred, y_true)

    # Initialize criterion without weights
    criterion_no_weights = IoULoss(num_classes=num_classes).to(device)
...
def test_iou_loss_with_ignore_index(sample_data):
    """
    Test IoULoss with ignored pixels.
    """
    y_pred, y_true = sample_data
    device = y_pred.device
    num_classes = y_pred.size(1)
    ...
    criterion = IoULoss(num_classes=num_classes, ignore_index=ignore_index).to(device)
...
def test_iou_loss_all_ignored(sample_data):
    """
...
def test_iou_loss_input_validation(sample_data):
    """
    Test input validation in IoULoss.
    """
    y_pred, y_true = sample_data
    device = y_pred.device
    num_classes = y_pred.size(1)
