...
from utils.io.colmap.load_colmap import Camera, Image, Point3D


def save_cameras_text(output_path: str | Path, cameras: Dict[int, Camera]) -> None:
    """Save COLMAP cameras to text format."""
...
def save_images_text(output_path: str | Path, images: Dict[int, Image]) -> None:
    """Save COLMAP images to text format."""
...
def save_points3D_text(output_path: str | Path, points3D: Dict[int, Point3D]) -> None:
    """Save COLMAP sparse points to text format."""
    path = Path(output_path)
    lines: List[str] = [
    ...
    for point_id in sorted(points3D):
        ...
        track_parts = []
        for image_idx, point2d_idx in zip(image_ids, point2d_ids):
...
def save_model_text(
    ...
    """Save a complete COLMAP model to text files."""
    output_root = Path(output_dir)
    save_cameras_text(output_root / "cameras.txt", cameras)
    save_images_text(output_root / "images.txt", images)
    save_points3D_text(output_root / "points3D.txt", points3D)


def save_cameras_binary(output_path: str | Path, cameras: Dict[int, Camera]) -> None:
    """Save COLMAP cameras to binary format."""
...
def save_images_binary(output_path: str | Path, images: Dict[int, Image]) -> None:
    """Save COLMAP images to binary format."""
...
def save_points3D_binary(output_path: str | Path, points3D: Dict[int, Point3D]) -> None:
    """Save COLMAP sparse points to binary format."""
    path = Path(output_path)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("wb") as handle:
...
def save_model_binary(
    ...
    """Save a complete COLMAP model to binary files."""
    output_root = Path(output_dir)
    save_cameras_binary(output_root / "cameras.bin", cameras)
    save_images_binary(output_root / "images.bin", images)
    save_points3D_binary(output_root / "points3D.bin", points3D)
