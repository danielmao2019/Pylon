...
def _check_camera_extrinsics_numpy(obj: Any) -> np.ndarray:
    ...
    ), "Rotation part of camera extrinsics must be orthogonal."
    assert np.isclose(
        np.linalg.det(rotation), 1.0
    ), "Rotation part of camera extrinsics must have determinant +1."
    return obj
...
def _check_camera_extrinsics_torch(obj: Any) -> torch.Tensor:
    ...
    assert torch.allclose(
        should_be_identity,
        torch.eye(3, dtype=obj.dtype, device=obj.device),
        atol=1.0e-06,
        rtol=0,
    ...
    assert torch.isclose(
        torch.det(rotation), torch.tensor(1.0, dtype=obj.dtype, device=obj.device)
    ), f"Rotation part of camera extrinsics must have determinant +1. det({rotation}) = {torch.det(rotation)}."
    return obj
