...
def filter_octree_gs_by_levels(
...
) -> Union[GaussianLoDModel, Tuple[GaussianLoDModel, Dict[str, Any]]]:
    ...
    Returns:
        If return_info is False:
            A new GaussianLoDModel with only the gaussians at the specified levels
        If return_info is True:
            Tuple of (filtered_model, info_dict) where:
                - filtered_model: GaussianLoDModel with only selected levels
    ...
    filtered_model._anchor_feat = nn.Parameter(
        filtered_model._anchor_feat.detach()[mask].clone()
    )
...
def render_rgb_from_octree_gs(
    ...
    """
    assert isinstance(model, GaussianLoDModel)
    ...
    if levels is not None:
        ...
        if return_info:
            dist = (
                torch.sqrt(
                    torch.sum((model.get_anchor - camera.camera_center) ** 2, dim=1)
                )
                * resolution_scale
            )
            pred_level = (
    ...
    if return_info:
        ...
        info_dict.update(
            {
                'anchor_mask_percentage': outputs.get('anchor_mask_percentage', 0.0),
                'anchor_mask_active': outputs.get('anchor_mask_active', 0),
                'anchor_mask_total': outputs.get('anchor_mask_total', 0),
            }
        )
