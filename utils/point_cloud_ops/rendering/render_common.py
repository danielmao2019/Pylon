...
def prepare_points_for_rendering(
    ...
    """Common preprocessing for point cloud rendering operations.

    Handles camera transformations and point projection that are shared
    across RGB, depth, and segmentation rendering. Input validation should
    be done separately via validate_rendering_inputs().
    ...
    Raises:
        AssertionError: If no points are visible after projection
...
def create_valid_mask(
    ...
    """Create a valid pixel mask from projected points.

    Args:
        points: Projected points tensor with shape [N, 3] containing (x, y, depth)
        resolution: Target resolution as (width, height) tuple
