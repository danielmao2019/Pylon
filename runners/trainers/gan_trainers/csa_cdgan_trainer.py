    ...
    def _train_step(self, dp: Dict[str, Dict[str, torch.Tensor]]) -> None:
        ...
        dp['labels'] = {
            'change_map': torch.eye(
                n=dp['outputs']['gen_image'].size(1),
                dtype=torch.float32,
                device=self.device,
                requires_grad=False,
            )[dp['labels']['change_map']].permute(0, 3, 1, 2),
        }
        g_loss = self.criterion.task_criteria['generator'](
            y_pred=dp['outputs'], y_true=dp['labels']
        )
        ...
        d_loss = self.criterion.task_criteria['discriminator'](
            y_pred=dp['outputs'], y_true=dp['labels']
        )
        d_loss.backward()
