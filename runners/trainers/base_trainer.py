...
class BaseTrainer(ABC):
    ...
    def _init_early_stopping(self) -> None:
        ...
        # building early stopping requires the metric it monitors
        assert (
            hasattr(self, 'metric') and self.metric is not None
        ), f"early_stopping is configured but no metric is available to build it; {self.config.get('metric', None)=}"
    ...
    def _init_model(self) -> None:
        ...
        if self.config.get('model', None):
            ...
            self.model = model
            if (
                getattr(self.model, 'initializer', None) is not None
                and self.cum_epochs == 0
            ):
                self.model.initializer(self.model)
        else:
            self.model = None
    ...
    def _init_checkpoint_indices(self) -> None:
        """Precompute epoch indices where checkpoints (and debug outputs) will be saved, keyed on config['checkpoint']['method']."""
        checkpoint_method = self.config['checkpoint']['method']
        ...
        if isinstance(checkpoint_method, int) and checkpoint_method > 0:
            # Interval-based: every N epochs, always including the last epoch
            ...
            return
        assert 0, f"Should not reach here. {checkpoint_method=}"
    ...
    def run(self) -> None:
        """Initialize components, save the init checkpoint when configured, run the train/val epoch loop with early stopping, then the test epoch."""
        # initialize run
        self._init_components_()
        # save the 0-epoch init checkpoint before the epoch loop
        if self.config['checkpoint'].get('save_init', False) and self.cum_epochs == 0:
            self._save_checkpoint_(
                output_path=os.path.join(self.work_dir, "checkpoint_init.pt")
            )
