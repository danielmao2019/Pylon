...
class SupervisedSingleTaskTrainer(BaseTrainer):
    ...
    def _init_optimizer(self) -> None:
        ...
        if not self.config.get('optimizer', None):
            self.logger.warning(
                "No optimizer specified in config, skipping optimizer initialization."
            )
            return
        # check dependencies
    ...
    def _init_scheduler(self):
        if not self.config.get('scheduler', None):
            self.logger.warning(
                "No scheduler specified in config, skipping scheduler initialization."
            )
            return
        # check dependencies
        ...
        assert hasattr(self.optimizer, 'optimizer') and isinstance(
