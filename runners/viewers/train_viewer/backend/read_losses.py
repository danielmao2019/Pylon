...
def read_losses(logs_dirpath: str) -> List[torch.Tensor]:
    ...
    assert isinstance(
        logs_dirpath, str
    ), f"logs_dirpath must be str, got {type(logs_dirpath)}"
    assert os.path.isdir(
        logs_dirpath
    ), f"logs_dirpath must exist and be path to a directory: {logs_dirpath}"
    ...
    while True:
        ...
        assert isinstance(
            losses, torch.Tensor
        ), f"Loaded losses must be torch.Tensor, got {type(losses)}"
        assert (
            losses.dim() == 1
        ), f"Losses tensor must be 1D, got {losses.dim()}D tensor with shape {losses.shape}"
        ...
        if expected_length is None:
            ...
            assert (
                len(losses) == expected_length
            ), f"All epochs must have same length. Expected {expected_length}, got {len(losses)} for epoch {idx}"
    ...
    assert (
        len(results) > 0
    ), f"No epoch directories with training_losses.pt found in {logs_dirpath}"
