...
class GANTrainer(BaseTrainer):
    ...
    def _train_step_(self, dp: Dict[str, Dict[str, Any]]) -> None:
        ...
        gen_image = self.model.netG(dp['inputs'])
        # update generator
        G_loss = self.criterion(self.model.netD(gen_image), real_tensor)
        self.optimizer.G_optimizer.zero_grad()
        G_loss.backward()
        self.optimizer.G_optimizer.step()
        # update discriminator
        D_loss = (
            self.criterion(self.model.netD(image), real_tensor) +
            self.criterion(self.model.netD(gen_image), fake_tensor)
        ) / 2
