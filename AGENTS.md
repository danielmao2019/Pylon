...
## 2. About how to execute task specs

1. for each task spec `task.md`, either it's a full task, or a sub-task of some task, there should always be a sibling file called `report.md`, unless a reporting markdown doc is otherwise specified by the task spec or by user prompt.
2. use this report.md frequently. use this as a communication interface with me during long task execution. maintain this `report.md` along the way, rather than dumping everything I need to know all at once upon task completion.
3. what I need to know (minimum):
   1. Decisions:
      1. The role you play: Think of me as the decision maker and you as the task executor. Eventually all decisions should be made by me. When you make a decision on your own, you should make it transparent and help me understand the task execution dynamic trajectory.
      2. Why you need to make decisions: I want you to have some level of independence so that you can fully complete the task and deliver it without me frequently intervening the process. This means there must be smaller decisions that you should make on your own.
      3. When you need to make decisions: When there is an under-specified item in the task spec and you are faced with multiple available options.
      4. What you should include in a report of a decision:
         1. list and describe each available option you face.
         2. name the choice you took.
         3. justify the choice you took.
      5. How to format a decision report in the `report.md`:
         1. each of such a decision made should go into a dedicated level-2 heading section in the report.md, or go into a dedicated level-3 heading section if the task is a sub-task.
         2. it should be removed when I point to one of those and tell you that I'm gonna revise this task spec to specify that concretely to override your choice. i.e., i made an under-specified item fully specified in the task spec.
   2. Mistakes:
      1. Types of mistakes: report all the mistakes you made, either you realized by yourself, or I pointed out and let you correct.
      2. What you should include in a report of a mistake:
         1. explain **what** the mistake was
         2. justify **why** it is wrong
         3. explain from your understanding what was the correct thing to do.
      3. How to format a mistake report in the `report.md`:
         1. each of these should go into a dedicated level-2 heading section in report.md, or level-3 in case of sub-task.
   3. Q/A:
      1. for the questions i asked you (not adhoc tasks i give you, but actual questions I asked) during the chat session.
      2. How to format a Q/A report in the `report.md`:
         1. each of these should go into a dedicated level-2 heading section in report.md, or level-3 in case of sub-task.

## 3. About coding style
...
2. every conversion of dtype on a PyTorch tensor or a numpy ndarray must be safe-guarded with before-conversion dtype assertion, to ensure you understand what is the dtype before conversion. you may assume that the conversion would do its job so no post-conversion assertion is needed.

### 3.2. How to write input validation
...
### 3.3. How to write input normalization
...
### 3.5. How to write steps and pipelines
...
### 3.6. How to write type annotations
...
### 3.7. About user code in this repo
...
### 3.9. How to write import statements
...
3. Function calls.
   1. If a function call has more than 3 arguments (positional, or keyword), you must call the function with `func(xxx=xxx, yyy=yyy, ...)`. i.e., ALL args should be called as keyword args. The reason is that with functions with many args, it is easy to mess up with the ordering.
   2. The ordering of the args when calling a function, and the order of the input validation assert statements in the function definition, must both follow the same order as the function args.
4. Doc strings.
   1. All functions and all methods should have doc strings.
   2. Doc strings must contain exactly three parts: description, args, and returns.
      1. Description must be concise.
      2. Args must be specific enough, especially on the data format convention, e.g., camera-to-world or world-to-camera of camera extrinsics, OpenGL or OpenCV or standard of camera extrinsics, shape of `torch.Tensor` and `numpy.ndarray`, dtype of `torch.Tensor` and `numpy.ndarray`, etc.
5. All assertions must have error messages. All error messages must contain two parts:
   1. A description of what happened in natural language.
   2. Debugging info, to some level. In most cases, you include the variables that contribute to the evaluation to False of the assertion. Some times, you may include more than those variables. There is no strict rule. Use common sense.
...
2. It is bad to make a function, or a method, that only does trivial things, because these functions/methods only adds unnecessary call stack to the program and does not contribute to code modularity, reusability, and such.
3. Some times there are multiple parts for the code of a function/method and the parts are actually independent. However, you often write code so that the lines for part A is mixed together with lines for part B, making the code hard to understand. You should learn to implement separation of concerns. You either create helper methods, or you make code chunks separated by symbolic comments like `# ---` or `# ===` or whatever.

### 3.13. First-Principle Rule
