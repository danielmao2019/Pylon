...
class ChangeStarCriterion(SingleTaskCriterion):
    ...
    def __call__(
        ...
        change_12_loss = self.criterion(
            y_pred=y_pred['change_12'], y_true=y_true['change']
        )
        change_21_loss = self.criterion(
            y_pred=y_pred['change_21'], y_true=y_true['change']
        )
        change_loss = 0.5 * (change_12_loss + change_21_loss)
        semantic_loss = self.criterion(
            y_pred=y_pred['semantic'], y_true=y_true['semantic']
        )
