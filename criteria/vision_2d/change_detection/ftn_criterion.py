...
from criteria.wrappers import AuxiliaryOutputsCriterion, HybridCriterion
...
class FTNCriterion(HybridCriterion):
    ...
    def __init__(self, num_classes: Optional[int] = 2, **kwargs) -> None:
        ...
        super(FTNCriterion, self).__init__(
            combine='sum', criteria_cfg=criteria_cfg, **kwargs
        )
    ...
    def __call__(
        ...
        # input checks
        assert (
            isinstance(y_pred, tuple) and len(y_pred) == 4
        ), f"{type(y_pred)=}, {len(y_pred)=}"
