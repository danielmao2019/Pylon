...
class CCDMCriterion(BaseCriterion):
    ...
    def theta_post(
        self,
        diffused_mask: torch.Tensor,
        original_mask: torch.Tensor,
        time: torch.Tensor,
    ) -> torch.Tensor:
        # input checks
        assert (
            len(diffused_mask) == len(original_mask) == len(time)
        ), f"{diffused_mask.shape=}, {original_mask=}, {time=}"
        assert (
            diffused_mask.ndim == original_mask.ndim + 1
        ), f"{diffused_mask.shape=}, {original_mask.shape=}"
        assert (
            diffused_mask.dtype == original_mask.dtype == time.dtype == torch.int64
        ), f"{diffused_mask.dtype=}, {original_mask.dtype=}, {time.dtype=}"
        assert (
            0 <= time.min() <= time.max() < self.num_steps
        ), f"{time=}, {self.num_steps=}"
        # transform original mask into one-hot encoding
        original_mask = to_one_hot(
            original_mask, num_classes=self.num_classes, ignore_value=self.ignore_value
        )
        assert (
            diffused_mask.shape == original_mask.shape
        ), f"{diffused_mask.shape=}, {original_mask.shape=}"
        assert (
            diffused_mask.shape[1] == original_mask.shape[1] == self.num_classes
        ), f"{diffused_mask.shape=}, {original_mask.shape=}"
        # compute theta post
        ...
        theta = (alphas_t * diffused_mask + (1 - alphas_t) / self.num_classes) * (
            alphas_cumprod_tm1 * original_mask
            + (1 - alphas_cumprod_tm1) / self.num_classes
        )
    ...
    def __call__(
        ...
        assert set(['original_mask', 'diffused_mask', 'time']).issubset(
            set(y_true.keys())
        ), f"{y_true.keys()=}"
        check_semantic_segmentation_true(obj=y_true['original_mask'])
        check_semantic_segmentation_true(obj=y_true['diffused_mask'])
        # compute loss
        prob_xtm1_given_xt_x0 = self.theta_post(
            diffused_mask=y_true['diffused_mask'],
            original_mask=y_true['original_mask'],
            time=y_true['time'],
        )
        ...
        loss = torch.nn.functional.kl_div(
            ...
            target=prob_xtm1_given_xt_x0,
            log_target=False,
            reduction='mean',
        )
        assert (
            not loss.isnan()
        ), f"{y_pred.min()=}, {y_pred.max()=}, {prob_xtm1_given_xt_x0.min()=}, {prob_xtm1_given_xt_x0.max()=}"
