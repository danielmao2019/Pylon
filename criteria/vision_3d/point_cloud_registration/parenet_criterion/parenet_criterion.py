...
class PARENetCriterion(BaseCriterion):
    ...
    def summarize(self, output_path: Optional[str] = None) -> Dict[str, float]:
        ...
        Returns:
            Dictionary with loss statistics matching DIRECTIONS keys
        """
        ...
        # Return dictionary matching DIRECTIONS keys
        return {
            "loss": avg_loss,
            "c_loss": avg_loss,      # For now, return total loss for all components
            "f_ri_loss": avg_loss,   # In future, could track individual components
            "f_re_loss": avg_loss
        }