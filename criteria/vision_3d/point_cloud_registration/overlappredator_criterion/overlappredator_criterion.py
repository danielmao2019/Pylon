...
class OverlapPredatorCriterion(SingleTaskCriterion):
    ...
    def __call__(
        self, y_pred: Dict[str, torch.Tensor], y_true: Dict[str, Any]
    ) -> torch.Tensor:
        """
        ...
        src_pcd = (
            torch.matmul(rot, src_pcd.transpose(0, 1)) + trans.reshape(-1, 1)
        ).transpose(0, 1)
        stats = dict()
        ...
        c_dist = torch.norm(
            src_pcd[correspondences[:, 0]] - tgt_pcd[correspondences[:, 1]], dim=1
        )
