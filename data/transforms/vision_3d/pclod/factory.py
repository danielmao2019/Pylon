...
def create_lod_function(
    ...
    if lod_type == "none":
        ...
        assert (
            lod_config is None or len(lod_config) == 0
        ...
        constructor_params = {
            k: v for k, v in lod_config.items() if k != "camera_state"
        }

        # Create continuous LOD function
        continuous_lod = ContinuousLOD(**constructor_params)
        return partial(continuous_lod.subsample, camera_state=camera_state)
        ...
        assert (
            point_cloud_id is not None
        ), "point_cloud_id is required for lod_type='discrete'"
