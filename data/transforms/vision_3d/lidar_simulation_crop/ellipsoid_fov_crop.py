...
class EllipsoidFOVCrop(BaseFOVCrop):
    """Ellipsoidal field-of-view cropping for LiDAR sensor simulation.
    
    Uses spherical coordinates (azimuth/elevation angles) to create ellipsoidal
    cropping regions that simulate real LiDAR sensor angular coverage limitations.
    This mode produces ellipsoidal coverage areas typical of spinning LiDAR sensors.
    
    Key characteristics:
    - Creates ellipsoidal coverage areas
    ...
    def _validate_fov_ranges(self, horizontal_fov: float, vertical_fov: float) -> None:
        """Validate FOV ranges for ellipsoidal cropping.
        
        Args:
            horizontal_fov: Horizontal FOV in degrees
            vertical_fov: Vertical FOV in degrees
        """
        # Ellipsoidal FOV supports full 360° horizontal coverage for spinning LiDAR
    ...
    def _apply_fov_constraints(self, sensor_frame_positions: torch.Tensor) -> torch.Tensor:
        """Apply ellipsoidal field-of-view constraints to points in sensor coordinate frame.
        
        Uses spherical coordinates to define ellipsoidal FOV regions typical of LiDAR sensors.
        
        Args:
            sensor_frame_positions: Point positions in sensor coordinate frame [N, 3]
            
        Returns:
            Boolean mask [N] indicating points within ellipsoidal FOV
        """
        ...
        # Apply horizontal FOV constraint - consistent usage of self.fov
        ...
        if horizontal_fov >= 360.0:
            ...
            half_h_fov = horizontal_fov / 2
            h_mask = torch.abs(azimuth_deg) <= half_h_fov
        
        # Apply vertical FOV constraint - consistent usage of self.fov
        half_v_fov = vertical_fov / 2
