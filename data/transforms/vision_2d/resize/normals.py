...
class ResizeNormals(BaseTransform):
    ...
    def __init__(self, target_size: Tuple[int, int]) -> None:
        ...
        assert (
            type(target_size[0]) == type(target_size[1]) == int
        ), f"{type(target_size[0])=}, {type(target_size[1])=}"
    ...
    def _call_single(self, normal: torch.Tensor) -> torch.Tensor:
        ...
        normal = torchvision.transforms.Resize(size=self.target_size, antialias=True)(
            normal
        )
