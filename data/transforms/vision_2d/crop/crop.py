...
class Crop(BaseTransform):
    ...
    def __init__(
        ...
        if not (
            ...
            and len(loc) == 2
            and all(isinstance(i, int) and i >= 0 for i in loc)
        ):
            raise ValueError(
                f"loc must be a tuple of two non-negative integers, but got {loc}"
            )
        if not (
    ...
    def _call_single(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        # Validate boundaries
        assert (
            0 <= x_start < x_end <= tensor.size(-1)
        ), "Crop x-coordinates are out of bounds."
        assert (
            0 <= y_start < y_end <= tensor.size(-2)
        ), "Crop y-coordinates are out of bounds."
