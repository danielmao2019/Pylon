...
class RandomRotation(BaseTransform):
    ...
    def __init__(
        ...
        """
        if (choices is None and range is None) or (
            choices is not None and range is not None
        ):
            raise ValueError(
                "Exactly one of `choices` or `range` must be provided, but got both None or both set."
            )

        if choices is not None:
            if not (
                isinstance(choices, list)
                and all(isinstance(d, (int, float)) for d in choices)
            ):
        ...
        if range is not None:
            if not (
                ...
                and len(range) == 2
                and all(isinstance(d, (int, float)) for d in range)
                and range[0] < range[1]
            ):
                raise ValueError(
                    "`range` must be a tuple of two integers (min, max) with min < max."
                )
