...
def calibrate_neighbors_stack_mode(
    ...
    neighbor_hists = torch.zeros(
        (num_stages, hist_n), dtype=torch.int64, device=dataset.device
    )
    ...
    neighbor_limits = torch.sum(
        (cum_sum < (keep_ratio * cum_sum[hist_n - 1, :])).to(torch.int64), dim=0
    )  # (num_stages,)
