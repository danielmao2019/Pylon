...
def test_whu_cd(dataset: torch.utils.data.Dataset) -> None:
    ...
    for idx in range(len(dataset)):
        ...
        img_1 = inputs['img_1']
        img_2 = inputs['img_2']
        assert type(img_1) == torch.Tensor and img_1.ndim == 2 and img_1.dtype == torch.float32
        assert type(img_2) == torch.Tensor and img_2.ndim == 2 and img_2.dtype == torch.float32
