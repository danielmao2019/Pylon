...
class COCOStuff164KDataset(BaseSemsegDataset):
    ...
    def _init_annotations(self) -> None:
        with open(
            os.path.join(
                self.data_root, 'curated', self.split, "Coco164kFull_Stuff_Coarse.txt"
            ),
            mode='r',
        ) as f:
        ...
        self.annotations = [
            {
                'image_filepath': os.path.join(
                    self.data_root, 'images', self.split, f"{id}.jpg"
                ),
                'label_filepath': os.path.join(
                    self.data_root, 'annotations', self.split, f"{id}.png"
                ),
            }
            for id in ids
        ]
    ...
    def _load_datapoint(self, idx: int) -> Tuple[
        ...
        meta_info = {
            'image_filepath': os.path.relpath(
                path=self.annotations[idx]['image_filepath'], start=self.data_root
            ),
            'label_filepath': os.path.relpath(
                path=self.annotations[idx]['label_filepath'], start=self.data_root
            ),
