...
function holdRollLockedGraphs(relayoutDataList) {
    ...
    function createRollLockCallback(graphElementId, worldAxis) {
        ...
        function holdSceneRollLock(scene) {
            ...
            const scenePlot = scene.plot;

            // Re-plots the scene through its own plot, which applies a new aspect ratio and new axis ranges, then holds the lock about the axis that aspect leaves before the scene draws its next frame. plotArgs are the arguments a figure update or relayout hands the scene's own plot.
            function rollLockedPlot(...plotArgs) {
                scenePlot.apply(scene, plotArgs);
                resolveSceneAxis(scene);
                rewriteHeldPose(scene.camera.view);
                return;
            }

            scene.plot = rollLockedPlot;
            return;
        }
        ...
        // Writes the pose a view holds back onto the lock about the current axis and discards every keyframe before it, so the renderer draws that pose from its next frame. The write goes through the controllers' wrapped rollLockedLookAt, and the view's flush hands the discard to each controller.
        function rewriteHeldPose(view) {
