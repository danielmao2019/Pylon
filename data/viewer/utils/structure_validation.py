...
def validate_3dcd_structure(datapoint: Dict[str, Any]) -> None:
    ...
    for pc_key in ['pc_1', 'pc_2']:
        ...
        # Validate point cloud dictionary
        pc = inputs[pc_key]
        assert isinstance(pc, dict), f"inputs['{pc_key}'] must be dict, got {type(pc)}"
        assert pc != {}, f"inputs['{pc_key}'] dict must not be empty"
        
        # Validate 'pos' field if present
        if 'pos' in pc:
            pos = pc['pos']
            assert isinstance(pos, torch.Tensor), f"inputs['{pc_key}']['pos'] must be torch.Tensor, got {type(pos)}"
            assert pos.ndim == 2 and pos.shape[1] == 3, f"inputs['{pc_key}']['pos'] must have shape [N,3], got {pos.shape}"
            assert pos.numel() > 0, f"inputs['{pc_key}']['pos'] must not be empty tensor"
    
    # Check for shape consistency between point clouds
    pc1, pc2 = inputs['pc_1'], inputs['pc_2']
    if 'pos' in pc1 and 'pos' in pc2:
        pos1, pos2 = pc1['pos'], pc2['pos']
        if isinstance(pos1, torch.Tensor) and isinstance(pos2, torch.Tensor):
