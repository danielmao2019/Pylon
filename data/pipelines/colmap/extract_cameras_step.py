...
class ColmapExtractCamerasStep(BaseStep):
    """Export complete NerfStudio JSON from COLMAP outputs."""
    ...
    def __init__(self, input_root: str | Path, output_root: str | Path) -> None:
        ...
        self.nerfstudio_path = self.output_root / "nerfstudio.json"

    def _init_input_files(self) -> None:
        self.input_files = ["0/cameras.bin", "0/images.bin"]

    def _init_output_files(self) -> None:
        self.output_files = ["nerfstudio.json"]
    ...
    def check_outputs(self) -> bool:
        ...
        if not outputs_ready:
            return False
        try:
            nerfstudio = NerfStudio.load(
                filepath=self.nerfstudio_path, device=torch.device("cpu")
            )
            ...
            logging.debug("COLMAP NerfStudio validation failed: %s", e)
            return False
        return True
    ...
    def run(self, kwargs: Dict[str, Any], force: bool = False) -> Dict[str, Any]:
        ...
        create_nerfstudio_from_colmap(
            filename="nerfstudio.json",
            colmap_cameras=colmap_data.cameras,
            colmap_images=colmap_data.images,
            output_dir=str(self.output_root),
            ply_file_path="sparse_pc.ply",
        )
        logging.info("   ✓ Wrote nerfstudio.json with %d frames", len(colmap_data.images))
        return {}
