...
def batch_neighbors_kpconv(
    ...
    neighbors = cpp_neighbors.batch_query(
        queries.detach().cpu().numpy().astype(np.float32),
...
def d3feat_collate_fn(list_data, config, neighborhood_limits):
    ...
    assert torch.all(
        valid_mask
    ), f"Invalid correspondences found ({(1-valid_mask.float().mean().item())*100}% invalid)"
    ...
    if sel_corr.shape[0] > max_corr_for_calibration:
        ...
        indices = torch.randperm(sel_corr.shape[0], device=device)[
            :max_corr_for_calibration
        ]
    ...
    batched_lengths = torch.tensor(
        [src_pc.num_points, tgt_pc.num_points], dtype=torch.int32, device=device
    )
