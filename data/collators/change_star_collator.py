...
class ChangeStarCollator(BaseCollator):
    ...
    def __init__(self, method: str, max_trails: Optional[int] = 50) -> None:
        ...
        if max_trails is not None:
            if not isinstance(max_trails, int) or max_trails <= 0:
                raise ValueError(
                    f"`max_trails` must be a positive integer. Got: {max_trails}"
                )
    ...
    def _shuffle(original: List[int], max_trails: int) -> List[int]:
        ...
        for _ in range(max_trails):
            ...
            if all(x != y for x, y in zip(original, proposal, strict=True)):
                return proposal
        raise RuntimeError(
            "Failed to shuffle without collisions after max_trails attempts."
        )
    ...
    def _call_train(
        ...
        datapoints["inputs"]["img_1"] = torch.stack(
            datapoints["inputs"]["img_1"], dim=0
        )
        ...
        datapoints["labels"]["semantic"] = torch.stack(
            datapoints["labels"]["lbl_1"], dim=0
        )
        del datapoints["labels"]["lbl_1"], datapoints["labels"]["lbl_2"]
        datapoints["labels"]["change"] = (
            datapoints["labels"]["semantic"]
            != datapoints["labels"]["semantic"][shuffled_indices]
        ).to(torch.int64)
    ...
    def _call_eval(
        ...
        datapoints["inputs"]["img_2"] = torch.stack(
            datapoints["inputs"]["img_2"], dim=0
        )
        assert set(datapoints["inputs"].keys()) == set(['img_1', 'img_2'])
        ...
        datapoints["labels"] = transpose_buffer(datapoints["labels"])
        assert set(datapoints['labels'].keys()) == set(['lbl_1', 'lbl_2'])
        datapoints["labels"]["semantic_1"] = torch.stack(
            datapoints["labels"]["lbl_1"], dim=0
        )
        datapoints["labels"]["semantic_2"] = torch.stack(
            datapoints["labels"]["lbl_2"], dim=0
        )
        del datapoints["labels"]["lbl_1"], datapoints["labels"]["lbl_2"]
        datapoints["labels"]["change"] = (
            datapoints["labels"]["semantic_1"] != datapoints["labels"]["semantic_2"]
        ).to(torch.int64)
        assert set(datapoints['labels'].keys()) == set(
            ['change', 'semantic_1', 'semantic_2']
        )
