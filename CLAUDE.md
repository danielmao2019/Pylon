...
Pylon enforces strict tensor type conventions across the entire framework (datasets, models, metrics, criteria, tests). The framework validates these conventions via `utils/input_checks/` module.

**Key principle**: Always specify `dtype` when creating tensors to match framework requirements.

## 5. About Testing

**For comprehensive testing guidelines, see `@docs/tests/`:**
- `@docs/tests/testing_philosophy.md` - Core testing principles and 9 test patterns
- `@docs/tests/test_implementation.md` - Implementation guidelines and best practices  
- `@docs/tests/conftest_guidelines.md` - Guidelines for using conftest.py files properly
- `@docs/tests/dummy_data.md` - Standardized dummy data generation for tests

**Key Testing Rules:**
- **Test directories should NOT contain `__init__.py` files** - they are not Python packages
