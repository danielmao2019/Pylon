...
def add_heading(config: str) -> str:
    heading = ""
    heading += "# This file is automatically generated by `./configs/benchmarks/change_detection/gen_change_star.py`.\n"
...
def main() -> None:
    ...
    config += f"from configs.common.datasets.change_detection.xview2 import config as dataset_config\n" 
    config += f"config.update(dataset_config)\n"
    config += '\n'
    # add model config
    config += f"# model config\n"
    config += f"from configs.common.models.change_detection.xview2.change_star import model_config\n"
    ...
    config += '\n'
    # add seeds
    relpath = os.path.join("change_detection", "xview2")
    seeded_configs: List[str] = utils.configs.generate_seeds(
        template_config=config,
        base_seed=os.path.join("benchmarks", relpath),
    )
    # save to disk
    ...
    for idx, seeded_config in enumerate(seeded_configs):
        ...
        seeded_config += f"config['work_dir'] = \"" + os.path.join("./logs/benchmarks", relpath, f"change_star_run_{idx}") + "\"\n"
        with open(os.path.join("./configs/benchmarks", relpath, f"change_star_run_{idx}.py"), mode='w') as f:
