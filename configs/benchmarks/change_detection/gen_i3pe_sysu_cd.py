...
def add_heading(config: str) -> str:
    heading = ""
    heading += "# This file is automatically generated by `./configs/benchmarks/change_detection/gen_i3pe_sysu_cd.py`.\n"
...
def main() -> None:
    ...
    config += f"from configs.common.datasets.change_detection.i3pe_sysu_cd import config as dataset_config\n" 
    ...
    for idx, seeded_config in enumerate(seeded_configs):
        ...
        seeded_config += f"config['work_dir'] = \"" + os.path.join("./logs", relpath, f"sysu_cd_run_{idx}") + "\"\n"
        with open(os.path.join("./configs", relpath, f"sysu_cd_run_{idx}.py"), mode='w') as f:
