...
def add_heading(config: str) -> str:
    heading = ""
    heading += "# This file is automatically generated by `./configs/benchmarks/change_detection/gen_ppsl.py`.\n"
...
def main() -> None:
    ...
    config += f"from configs.common.datasets.change_detection.val.levir_cd import config as val_dataset_config\n" 
    ...
    for idx, seeded_config in enumerate(seeded_configs):
        ...
        seeded_config += f"config['work_dir'] = \"" + os.path.join("./logs", relpath, f"ppsl_whu_bd_levir_cd_run_{idx}") + "\"\n"
        with open(os.path.join("./configs", relpath, f"ppsl_whu_bd_levir_cd_run_{idx}.py"), mode='w') as f:
