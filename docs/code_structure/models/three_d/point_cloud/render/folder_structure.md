...
├── test_render_depth.py  # depth rendering: resolution, the valid mask, occlusion order even where many points share a pixel, the Cameras batch axis with its per-camera equality and cull, point-size dilation over a finite or NaN background, and the inputs it refuses
