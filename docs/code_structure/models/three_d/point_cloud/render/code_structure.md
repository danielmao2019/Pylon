...
└── def render_rgb_from_rendering_points(rendering_points: torch.Tensor, original_data_indices: torch.Tensor, pc: PointCloud, resolution: Tuple[int, int], ignore_value: float = 0.0) -> torch.Tensor
    ...
    ├── if colors is an integer dtype, or its values sit in [0, 255] with a max above 1.0
    ...
    └── return  # rgb_image [3, H, W], covered pixels in [0, 1] and the rest at ignore_value
```
...
└── def render_mask_from_rendering_points(rendering_points: torch.Tensor, resolution: Tuple[int, int], device: torch.device) -> torch.Tensor
    ├── # Marks which pixels of the image any projected point landed on.
...
└── def render_rgb_from_pointcloud_o3d(pc_data: PointCloud, camera: Camera, resolution: Tuple[int, int]) -> torch.Tensor
    ...
    ├── if pc_data.rgb is an integer dtype and its values sit in [0, 255] with a max above 1.0
    │   └── impls divide colors by 255.0
...
│   ├── impls downscale_factor = the valid factor of {1, 2, 4, 8} nearest the native-over-render resolution ratio
│   ├── calls prepare_points_for_rendering(pc=pc, camera=camera, resolution=resolution)
│   ├── calls Select(indices=image_plane_points_indices)
│   ├── impls pc = that selector applied to pc, keeping only the points that reached the image plane
...
│   ├── else
│   │   ├── calls tempfile.TemporaryDirectory()
│   │   └── impls tempdir = its name, and cleanup_fn = its cleanup
│   ├── try
...
│   │   ├── calls Camera(intrinsics=camera.intrinsics, extrinsics=CameraExtrinsics(extrinsics=aux_standard, extr_convention="standard", device=device), device=device).to(extr_convention=camera.extrinsics.extr_convention)  # camera was rebound to "standard" above, so this target convention is "standard"
│   │   └── impls append the resulting aux_camera to the auxiliary list
│   └── return  # the 26 auxiliary cameras
...
│       ├── impls array = the clamped [3, H, W] tensor as an 8-bit [H, W, 3] numpy array
│       └── calls Image.fromarray(array).save(file_path)  # image_{idx:02d}.png under the images{suffix} directory
...
│       ├── impls array = the boolean [H, W] mask as an 8-bit 0/255 numpy array
│       └── calls Image.fromarray(array, mode="L").save(file_path)  # mask_{idx:02d}.png under the masks{suffix} directory
...
│   ├── impls assert at least one config.yml exists under the output directory
...
│   ├── calls camera.to(device=points.device, extr_convention="opencv").scale_intrinsics(resolution=resolution)
│   ├── impls camera_prepared = the resolution-scaled opencv camera that returns
...
│   │   │   ├── impls increment n and empty the CUDA cache to retry with a halved batch
│   │   │   └── continue
│   │   └── except Exception
│   │       └── raise  # any non-OOM failure is not a batch-size problem, so it propagates
...
│   ├── impls extrinsics = camera.extrinsics.extrinsics  # the [4, 4] cam2world tensor, in the opencv convention the public entry prepared
...
│   │   ├── if pts.numel() == 0
│   │   │   └── continue
│   │   └── impls append pts, and idx offset by the batch start i so the indices are global
...
│   ├── impls drop the per-batch references and empty the CUDA cache
...
│   ├── calls world_to_camera_transform(points=current_points, extrinsics=extrinsics, inplace=True, max_divide=2)  # the world-to-camera step
...
└── def apply_point_size_postprocessing(rendered_image: torch.Tensor, depth_map: torch.Tensor, point_size: float, ignore_value: Union[int, float] = 0.0) -> torch.Tensor
    ├── # Grows every rendered point to the requested diameter by dilating each pixel from a neighbour of the original depth map.
    ├── if point_size <= 1.0
    ...
    │   ├── impls propagate_mask = the neighbour carries depth, and the current pixel is empty or the neighbour is nearer in the unmodified depth_map
...
└── def create_circular_kernel_offsets(point_size: float, device: torch.device) -> torch.Tensor
    ├── # Enumerates the (y, x) offsets of the disc of the given diameter, the neighbourhood a rendered point is dilated over.
    ├── impls kernel_size = the ceiling of point_size, kernel_radius = half of point_size
    ├── impls y_kernel, x_kernel = the meshgrid over arange(kernel_size) - kernel_size // 2, which is off-centre for an even kernel_size
