...
│   ├── impls rot_avg = the mean of each edge's two endpoint rotations
│   ├── impls term = rot_avg applied to reference_edge_vectors
│   ├── impls weighted_term = weights * term
│   ├── impls rhs = each vertex's accumulation of weighted_term over its incident edges, signed by edge direction
│   ├── impls rhs = rhs plus lambda_c times constraint_mask times targets
...
└── def compute_arap_energy(verts: torch.Tensor, edge_vertex_indices: torch.Tensor, weights: torch.Tensor, reference_edge_vectors: torch.Tensor, rotations: torch.Tensor, constraint_mask: torch.Tensor, targets: torch.Tensor, lambda_c: float) -> torch.Tensor
    ...
    ├── impls edge_vec = verts differenced across edge_vertex_indices
    ├── impls rot_avg = the mean of each edge's two endpoint rotations
    ├── impls rotated = rot_avg applied to reference_edge_vectors
    ├── impls residual = edge_vec - rotated
    ├── impls edge_energy = weights times the per-edge squared norm of residual
    ├── impls constraint_energy = lambda_c times constraint_mask times the per-vertex squared norm of verts - targets
    └── return edge_energy summed plus constraint_energy summed  # the scalar total ARAP energy
```
...
│   ├── impls ab = b - a
│   ├── impls ac = c - a
│   ├── impls cross = torch.cross(ab, ac, dim=1)
│   └── return the ab·ac row sums over torch.linalg.norm(cross, dim=1)  # cos over sin at a
...
│   ├── impls vi = verts at each edge's first endpoint
│   ├── impls vj = verts at each edge's second endpoint
│   ├── impls weighted = weights broadcast over vi - vj
│   ├── impls result = a zeros tensor of verts' shape
│   ├── impls scatter-add weighted into result at each edge's first endpoint
│   ├── impls scatter-subtract weighted from result at each edge's second endpoint
│   └── return result  # [V, 3]
...
│   ├── impls src, dst = each edge's two endpoint index arrays
│   ├── impls rows = src, dst, src, dst concatenated
│   ├── impls cols = src, dst, dst, src concatenated
│   ├── impls data = weights, weights, -weights, -weights concatenated
│   ├── impls laplacian_matrix = a scipy COO matrix of data at (rows, cols), num_verts square, converted to CSC
│   └── return laplacian_matrix  # [num_verts, num_verts]
...
│   ├── impls constraint_np = constraint_mask as a float64 array scaled by lambda_c
│   └── return a scipy CSC diagonal matrix over constraint_np  # [V, V]
...
└── def solve_factorized_sparse_system(factorized_system: Any, rhs: torch.Tensor, device: torch.device, dtype: torch.dtype) -> torch.Tensor
    ...
    ├── impls rhs_np = rhs detached onto the cpu as a float64 array
    ├── impls solution_np = np.stack of the factorized system's solve over each column of rhs_np
...
│   ├── impls normal_norm = the per-row L2 norm of normals
│   ├── impls assert every normal_norm entry is above 0
│   └── return normals divided by normal_norm  # [V, 3], L2-normalized per row
...
└── def build_topology_edges_from_faces(faces: torch.Tensor) -> torch.Tensor
    ...
    ├── impls topology_edges = each face's three corner pairs, stacked into one [3F, 2] index tensor
    ├── impls sort each row so the smaller vertex index comes first  # one canonical direction per undirected edge
    └── return the unique rows of topology_edges, in sorted order    # [E, 2]
```
