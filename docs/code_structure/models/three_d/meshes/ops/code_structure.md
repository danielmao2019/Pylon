...
└── def compute_arap_energy(verts: torch.Tensor, edge_vertex_indices: torch.Tensor, weights: torch.Tensor, reference_edge_vectors: torch.Tensor, rotations: torch.Tensor, constraint_mask: torch.Tensor, targets: torch.Tensor, lambda_c: float) -> torch.Tensor
    ...
    ├── impls edge_energy = the weighted squared norm of current_edge_vectors - rotated_reference, per edge
    ├── impls constraint_energy = lambda_c * the squared norm of verts - targets at the vertices constraint_mask selects, per vertex
    ├── impls total_energy = edge_energy summed over edges plus constraint_energy summed over vertices
    └── return total_energy  # the scalar total ARAP energy
```
...
│   ├── impls cotangent_value = the dot product of u with v over sine_term  # cos over sin at a
...
│   ├── impls index-add contributions into result at each edge's first endpoint
│   ├── impls index-add their negation into result at each edge's second endpoint
│   └── return result  # [V, 3]
...
│   ├── impls laplacian_matrix = a scipy COO matrix of num_verts square over both entry sets, converted to CSC
...
│   ├── impls constraint_matrix = a scipy CSC diagonal matrix over diagonal
│   └── return constraint_matrix  # [V, V]
...
│   ├── impls factorized_system = the scipy splu factorization of system_matrix
...
└── def solve_factorized_sparse_system(factorized_system: Any, rhs: torch.Tensor, device: torch.device, dtype: torch.dtype) -> torch.Tensor
    ...
    ├── for each column of rhs_np
    │   └── impls solve that column with the factorized system
    ├── impls solution_np = those column solutions stacked back into rhs's column layout
    ├── impls solution = solution_np as a torch tensor of dtype on device
    └── return solution  # rhs's own shape
```
...
│   ├── impls normals = normals L2-normalized per row
│   └── return normals  # [V, 3]
...
└── def build_topology_edges_from_faces(faces: torch.Tensor) -> torch.Tensor
    ...
    ├── impls sort each row of topology_edges so the smaller vertex index comes first  # one canonical direction per undirected edge
    ├── impls topology_edges = its unique rows, in sorted order
    └── return topology_edges  # [E, 2]
```
