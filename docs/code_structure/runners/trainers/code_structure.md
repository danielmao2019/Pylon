...
└── class BaseTrainer(ABC)
    ...
    │   ├── impls self.eval_n_jobs = config.get('eval_n_jobs', 1)
    │   ├── calls self._init_work_dir
    │   ├── calls self._init_tot_epochs
    │   ├── impls enable torch.autograd anomaly detection
    │   ├── impls self.after_train_thread = None
    │   ├── impls self.after_val_thread = None
    │   └── impls self.buffer_lock = threading.Lock()
    ├── def run(self) -> None
    │   ├── # Initializes components, saves the init checkpoint when configured, runs the train/val epoch loop with early stopping, then the test epoch.
    │   ├── calls self._init_components_
    │   ├── if self.config['checkpoint'].get('save_init', False) and self.cum_epochs == 0
    │   │   └── calls self._save_checkpoint_                 # output_path = the 0-epoch init checkpoint
    ...
    │   │   └── impls self.work_dir = the work_dir
    │   └── else
    ...
    │   ├── impls self.train_seeds = the config's train_seeds
    │   ├── impls self.val_seeds = the config's val_seeds
    │   ├── impls self.test_seed = the config's test_seed
    │   └── calls set_seed                               # the init seed
    ├── def _init_checkpoint_indices(self) -> None
    │   ├── # Precomputes the epoch indices at which checkpoints and debug outputs are saved, keyed on config['checkpoint']['method'].
    ...
    │   │   ├── impls self.cum_epochs = 0
    │   │   └── return
    │   └── impls self.cum_epochs = load_idx + 1
    ...
    │   ├── else
    │   │   └── impls self.train_dataloader = None
    ...
    │   ├── else
    │   │   └── impls self.val_dataloader = None
    ...
    │   └── else
    │       └── impls self.test_dataloader = None
    ...
    │   ├── # Builds the model from config (its initializer built with it as a sub-component), moves it to device, and — training from scratch — applies the model's own initializer that sets its initial state.
    │   ├── if self.config.get('model', None)
    │   │   ├── calls build_from_config                  # the model, with its nested initializer sub-component
    │   │   ├── impls assert isinstance torch.nn.Module
    │   │   ├── impls move the model to self.device
    │   │   └── if getattr(self.model, 'initializer', None) is not None and self.cum_epochs == 0
    │   │       └── impls apply self.model.initializer to self.model
    │   └── else
...
```text
config
├── work_dir              # str — the training output root
├── epochs                # int — the total epoch count
├── init_seed             # int — the determinism seed set once at init
├── train_seeds           # List[int] — the per-epoch training seeds
├── val_seeds             # List[int] — the per-epoch validation seeds
├── test_seed             # int — the test-epoch seed
├── checkpoint            # the checkpoint group — sibling of model / criterion / metric / early_stopping
│   ├── method            # 'all' | 'latest' | every-N — which epoch checkpoints survive cleaning
│   └── save_init         # bool — also save the 0-epoch init checkpoint
├── train_dataset         # {class, args}
├── train_dataloader      # {class, args}
├── val_dataset           # {class, args}
├── val_dataloader        # {class, args}
├── test_dataset          # {class, args}
├── test_dataloader       # {class, args}
├── criterion             # {class, args} — a criteria.BaseCriterion nn.Module
├── metric                # {class, args}
├── model                 # {class, args} — an nn.Module; args also carry the model's own constructor args
│   └── args
│       └── initializer   # {class, args} — the custom init, a model sub-component the trainer applies at epoch 0
├── optimizer             # {class, args} — over model.parameters()
├── scheduler             # {class, args} — the LR scheduler
├── early_stopping        # {class, args} — optional
├── debugger              # {class, args} — optional
└── eval_n_jobs           # int — parallel eval workers, 1 = sequential
```
