...
└── class BaseTrainer(ABC)
    ...
    │   ├── impls copy.deepcopy the config onto self.config and store device / eval_n_jobs
    ...
    │   │   └── impls os.makedirs and store the work_dir
    │   └── else
    ...
    │   │   ├── impls assert isinstance criteria.BaseCriterion and torch.nn.Module
    ...
    │   │   ├── impls os.makedirs the epoch root and summarize the criterion / optimizer buffers to disk
    ...
    │   │   ├── impls os.makedirs the epoch root and summarize the metric scores to disk
    ...
    │   ├── if checkpoint_method == 'all'
    │   │   └── return
    │   ├── impls build the keep-set from latest, best, and checkpoint_indices
