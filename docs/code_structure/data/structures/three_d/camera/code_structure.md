...
│   ├── for each camera
│   │   └── calls camera_vis(camera=camera, frustum_size=frustum_size, frustum_color=frustum_color, point_size=point_size, point_color=point_color)
│   └── return
...
│   ├── def _validate_inputs [local]
│   │   └── impls asserts format is a non-empty str  # drawn because this is format's only owner on this path
│   ├── calls _validate_inputs
│   ├── def _normalize_inputs [local]
│   │   ├── impls single = isinstance(cameras, Camera)
│   │   ├── if single
...
│   ├── calls _validate_inputs
│   ├── def _normalize_inputs [local]
│   │   ├── calls _normalize_format  # ahead of the payload, since which payload shape counts as single is the format's own
│   │   ├── calls _normalize_payload_to_plural
│   │   ├── impls device = torch.device(device) if device is not None else torch.device("cpu")
│   │   └── return payload, device, format, single
│   ├── calls _normalize_inputs
│   ├── def _deserialize [local]
│   │   ├── # Map the plural payload the requested format spells back to the plural Cameras.
│   │   ├── if format == "json"
│   │   │   └── return _deserialize_cameras_json(per_camera_dicts=payload, device=device)
│   │   ├── if format == "npz"
...
│   ├── if format == "json"
│   │   ├── impls single = isinstance(payload, dict)
│   │   ├── if single
│   │   │   └── impls payload = that bare per-camera dict wrapped in a list
│   │   └── return payload, single
│   ├── if format == "npz"
│   │   ├── impls single = the payload's arrays carry no leading batch axis
│   │   ├── if single
...
└── def _normalize_format(format: str) -> str
    ...
    ├── impls strips the surrounding whitespace from format
    ├── impls asserts the stripped format is non-empty
    ├── if format starts with "."
    │   └── impls drops that leading "."
    ├── impls asserts the normalized format is in _CAMERA_SERIALIZATION_FORMATS
    └── return format
```
