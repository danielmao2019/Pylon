...
│   ├── impls asserts f > 0 and cx >= 0 and cy >= 0
│   └── return params
...
│   ├── impls asserts fx > 0 and fy > 0 and cx >= 0 and cy >= 0
│   └── return params
└── def _validate_camera_intrinsics_params_ortho(params: Any) -> Dict[str, Union[int, float]]
    ...
    ├── impls asserts params is a Dict[str, Union[int, float]] with exactly keys {fx, fy, cx, cy}
    ├── impls asserts fx > 0 and fy > 0 and cx and cy are finite
    └── return params
```
...
└── def validate_camera_attributes(intrinsics: "CameraIntrinsics", extrinsics: "CameraExtrinsics", name: Optional[str], id: Optional[int], device: Union[str, torch.device]) -> None
    ...
    ├── impls asserts isinstance(intrinsics, CameraIntrinsics) and isinstance(extrinsics, CameraExtrinsics)
    ├── impls asserts name is None or a str, and id is None or an int
    ├── impls asserts device is a valid torch device spec
    └── return
```
...
│   ├── def project(self, points_camera: torch.Tensor) -> torch.Tensor   [abstract]
...
│   │   ├── impls scale the focal params (f / fx / fy) and the principal-point params (cx, cy) in a copy of self.params by (sx, sy)
...
│   │   └── # The shared focal length params["f"].
│   ├── def project(self, points_camera: torch.Tensor) -> torch.Tensor   [override]
│   │   ├── # Perspective projection with a single shared focal length.
│   │   ├── impls image_xy = f * points_xy / points_z + (cx, cy)
│   │   └── return  # the [..., 2] image points
...
│   │   ├── impls image_xy = (fx, fy) * points_xy / points_z + (cx, cy)
│   │   └── return  # the [..., 2] image points
│   └── def fov(self) -> Tuple[float, float]                          # @property
│       ├── # The horizontal / vertical field of view in degrees (perspective model only).
...
│       ├── impls image_xy = (fx, fy) * points_xy + (cx, cy)
│       └── return  # the [..., 2] image points
...
│   │   └── impls moves the extrinsics to device and stores _extrinsics, _convention, _device
...
└── class Camera
    ...
    │   └── impls moves the intrinsics and extrinsics to device and stores _intrinsics, _extrinsics, _name, _id, _device
...
└── class Cameras
    ...
    │   └── impls moves each CameraIntrinsics / CameraExtrinsics to device and stores _intrinsics, _extrinsics, _names, _ids, _device, _name_to_index
...
└── def camera_vis(camera: Camera, frustum_size: Optional[float] = None, frustum_color: Optional[Tuple[int, int, int]] = None, point_size: Optional[float] = None, point_color: Optional[Tuple[int, int, int]] = None) -> Dict[str, Any]
    ...
    ├── impls computes the center marker from the camera's extrinsics center, its color from point_color, and its size from point_size
    ├── impls computes axes and frustum lines colored by frustum_color from the camera's extrinsics right / forward / up, the camera's intrinsics, and frustum_size
    └── return
```
...
│   │   └── impls builds that camera's json dict from intrinsics.model, intrinsics.params, extrinsics.extrinsics, extrinsics.convention, name, and id
│   └── return
...
│   │   ├── impls asserts the keys match _CAMERA_JSON_KEYS and the model / convention / name / id field types
...
│   ├── for each camera in cameras
│   │   └── impls appends that camera's model, params (json-encoded), extrinsics, convention, name, and id to the batch
...
│   │   ├── impls decodes that index's model, params, extrinsics, convention, name, and id (resolving has_name / has_id flags and the -1 id sentinel) on device
│   │   ├── calls build_camera_intrinsics            # model + params -> the model's CameraIntrinsics subclass (validates model + params)
