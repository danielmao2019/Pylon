...
│   ├── impls cameraState = the CameraState: perspective-three intrinsics off the camera, the converted position, quaternion, target and up, three_trackball conventions, null name and id  # impls-node-one-step:skip — one record's fields
│   └── return cameraState
...
│   ├── impls vectorRecord = the record holding the vector's x, y and z  # impls-node-one-step:skip — one record's fields
│   └── return vectorRecord
...
│   ├── impls quaternionRecord = the record holding the quaternion's x, y, z and w  # impls-node-one-step:skip — one record's fields
