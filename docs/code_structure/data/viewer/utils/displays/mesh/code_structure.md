...
│   ├── impls figure = a go.Figure holding one Mesh3d over the verts and faces, vertex-colored by those CSS colors, smooth-shaded under the fixed lighting and light position, hover skipped, named title
...
│   └── return  # the html.Iframe id'd by component_id, serving iframe_html under TEXTURED_MESH_IFRAME_STYLE and those attributes
...
│   │   └── return  # viewer_script_template with its position, uv, texture, bounds, camera-sync, default eye-z and viewer-id placeholders filled
...
│   └── return  # that file's text, read as utf-8
...
│   └── return  # the HTML document: the title, the three.js script and extra_script_tags_block, the full-bleed styles, an empty #mesh-root div, and viewer_script in a script element after it
...
│   └── impls figure.update_layout(meta {"meshViewBounds": mesh_view_bounds}, title, the margins and backgrounds, the data-aspect scene with hidden axes over those axis ranges, framed from PLOTLY_MESH_DEFAULT_CAMERA_EYE_Z with +Y up, no legend, uirevision PLOTLY_MESH_CAMERA_UIREVISION)
...
│   └── return  # the geometric mean of positive_extent
...
│   ├── impls trace_kwargs = the verts' x, y and z with the faces' i, j and k, at effective_opacity, smooth-shaded, hover skipped
│   ├── if effective_color is a list
│   │   └── impls trace_kwargs["vertexcolor"] = effective_color
│   ├── else
│   │   └── impls trace_kwargs["color"] = effective_color
│   └── return  # the go.Mesh3d over trace_kwargs
...
│   └── return  # that triplet as `rgb(r,g,b)`
...
│   └── return  # that base64 under the PNG data-url prefix
...
│   │   └── return container
│   ├── impls leaf = the LeafVNode keyed by displayResponse.url or `mesh:${displayResponse.slot_id}`, with empty props and that render
│   └── return leaf
...
│   └── return { verts: parsed.verts, faces: parsed.faces, texture }
...
│   ├── else
│   │   └── impls {}
│   ├── impls material = new THREE.MeshBasicMaterial(vertexColors useVertexColors, side effectiveSide, opacity effectiveOpacity, transparent when effectiveOpacity < 1 or the vertex colors carry alpha, spread with those map and color entries)
...
│   │   └── return { kind: "uv_texture_map", uvTextureMap, vertsUvs: parsed.vertsUvs, facesUvs: parsed.facesUvs }
...
│   ├── impls useUvs = sawAnyUv and vtCoords is non-empty
...
│   │       └── if parts holds 2 entries or more
│   │           └── return  # the parts past the first, joined on a space
│   └── return null
...
│   ├── if slash < 0
│   │   └── return siblingName
│   └── return  # primaryUrl through that slash, followed by siblingName
...
│   ├── if the response is not ok
│   │   └── throw GET ${url} failed: ${response.status}
│   ├── impls raw = the awaited response body parsed as JSON, read as its geometry_url, indices and values fields
...
│   └── return  # { geometryUrl: raw.geometry_url, indices as an Int32Array, values as a Float32Array }
...
│   └── return { verts: parsed.verts, faces: parsed.faces, texture: { kind: "vertex_color", vertexColor } }
