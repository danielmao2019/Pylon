...
│   ├── impls sceneGraphObject = { object: group, labels, labelOverlay }
...
│   ├── impls payload = the awaited response body parsed as JSON, cast unchecked to SceneGraphPayload
│   └── return payload
...
│   │   ├── impls label = { node, position: new THREE.Vector3(entry.position.x, entry.position.y, entry.position.z) }
│   │   └── return label
│   ├── impls labels = payload.labels mapped through that step
│   ├── impls sceneGraphPoints = { points, labels }
...
└── function _projectLabelsOntoOverlay({ camera, labels, labelOverlay, labelFontSize, labelColor, }: { camera: THREE.PerspectiveCamera; labels: object[]; labelOverlay: HTMLDivElement; labelFontSize?: number; labelColor?: string; }): void
    ...
    └── for each label in labels
        ├── impls (node, position) = the label, read as its overlay node and world position  # impls-node-one-step:skip — one destructuring's two fields
