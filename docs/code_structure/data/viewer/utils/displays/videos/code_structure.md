...
└── def create_video_display(src: str | None, title: str) -> html.Div
    ...
    ├── if src is None
    │   └── return  # the placeholder-surface html.Div standing in for the missing video
    └── return  # the html.Div holding the html.Video built from src, controls enabled, and title
```
