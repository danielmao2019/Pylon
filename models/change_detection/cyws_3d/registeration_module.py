...
class FeatureRegisterationModule(nn.Module):
    ...
    def register_3d_features(
        ...
        if sum(using_camera_parameters) > 0:
            _K_inv_1, _K_inv_2, _Rt_1_to_2, _Rt_2_to_1 = (
                estimate_Rt_using_camera_parameters(
                    torch.stack(
                        [
                            batch["intrinsics1"][i]
                            for i, x in enumerate(using_camera_parameters)
                            if x
                        ]
                    ),
                    torch.stack(
        ...
        nearest_resize = K.augmentation.Resize(
            features1.shape[-2:], resample=0, align_corners=None, keepdim=True
        )
            ...
            return geometry.convert_world_to_image_coordinates(
                geometry.convert_image_coordinates_to_world(
                    ...
                    depth=geometry.sample_depth_for_given_points(
                        batch["depth_1"][index_in_batch].unsqueeze(0), points
                    ),
        ...
        def transform_points_2_to_1(points, index_in_batch):
            ...
            return geometry.convert_world_to_image_coordinates(
                geometry.convert_image_coordinates_to_world(
                    image_coords=points,
                    depth=geometry.sample_depth_for_given_points(
                        batch["depth_2"][index_in_batch].unsqueeze(0), points
                    ),
    ...
    def register_2d_features(self, batch, features1, features2):
        ...
        image1_points_warped = geometry.transform_points(
            M_1_to_2, image_coords, keep_depth=True
        )
        image2_points_warped = geometry.transform_points(
            M_2_to_1, image_coords, keep_depth=True
        )
        image1_warped_onto_image2 = self.feature_warper.render_features_from_points(
            image1_points_warped, features1
        )
        image2_warped_onto_image1 = self.feature_warper.render_features_from_points(
            image2_points_warped, features2
        )

        def transform_points_1_to_2(points, index_in_batch):
            points = points.unsqueeze(0)
            return geometry.transform_points(
                M_1_to_2[index_in_batch].unsqueeze(0), points, keep_depth=False
            )[0]

        def transform_points_2_to_1(points, index_in_batch):
            points = points.unsqueeze(0)
            return geometry.transform_points(
                M_2_to_1[index_in_batch].unsqueeze(0), points, keep_depth=False
            )[0]
    ...
    def register_identity_features(self, batch, features1, features2):
        ...
        image1_warped_onto_image2 = self.feature_warper.render_features_from_points(
            image_coords, features1
        )
        image2_warped_onto_image1 = self.feature_warper.render_features_from_points(
            image_coords, features2
        )
    ...
    def register_features(self, batch, image1, image2, strategy):
        ...
        if strategy == "3d":
            ...
            (
                image1_warped_onto_image2,
                image2_warped_onto_image1,
                trasform_points_1_to_2,
                transform_points_2_to_1,
            ) = self.register_3d_features(
                batch,
                torch.cat([image1, visibility], dim=1),
                torch.cat([image2, visibility], dim=1),
            )
            ...
            (
                ...
                trasform_points_1_to_2,
                transform_points_2_to_1,
            ) = self.register_identity_features(batch, image1, image2)
    ...
    def forward(self, batch, image1, image2):
        ...
        image1_3d, image2_3d, trasform_points_1_to_2_3d, transform_points_2_to_1_3d = (
            self.register_features(
                slice_batch_given_bool_array(batch, reg_3d),
                image1[reg_3d],
                image2[reg_3d],
                "3d",
            )
        )
        image1_2d, image2_2d, trasform_points_1_to_2_2d, transform_points_2_to_1_2d = (
            self.register_features(
                slice_batch_given_bool_array(batch, reg_2d),
                image1[reg_2d],
                image2[reg_2d],
                "2d",
            )
        )
        image1_id, image2_id, trasform_points_1_to_2_id, transform_points_2_to_1_id = (
            self.register_features(
                slice_batch_given_bool_array(batch, reg_id),
                image1[reg_id],
                image2[reg_id],
                "identity",
            )
...
def estimate_Rt_using_camera_parameters(
    intrinsics1, intrinsics2, rotation1, rotation2, position1, position2
):
...
def estimate_Rt_using_points(points1, points2, depth1, depth2):
    K_inv, Rt = geometry.setup_canonical_cameras(
        len(points1), tensor_to_infer_type_from=points1[0]
    )
    ...
    for i in range(len(points1)):
        points1_in_world_coordinates = geometry.convert_image_coordinates_to_world(
            ...
            depth=geometry.sample_depth_for_given_points(
                depth1[i].unsqueeze(0), points1[i].unsqueeze(0)
            ),
        ...
        points2_in_world_coordinates = geometry.convert_image_coordinates_to_world(
            ...
            depth=geometry.sample_depth_for_given_points(
                depth2[i].unsqueeze(0), points2[i].unsqueeze(0)
            ),
            K_inv=K_inv[i].unsqueeze(0),
            Rt=Rt[i].unsqueeze(0),
        ).squeeze(0)
...
def slice_batch_given_bool_array(batch, mask):
    ...
    for key in batch.keys():
        ...
        if isinstance(batch[key], list):
            sliced_batch[key] = [
                batch[key][i] for i in range(len(batch[key])) if mask[i]
            ]
            if "bbox" in key or "point" in key:
                continue
            if len(sliced_batch[key]) > 0 and isinstance(
                sliced_batch[key][0], torch.Tensor
            ):
...
class DifferentiableFeatureWarper(nn.Module):
    ...
    def render(self, point_cloud, device, image_hw):
        ...
        canonical_rasterizer = PointsRasterizer(
            cameras=canonical_cameras, raster_settings=raster_settings
        )
        canonical_renderer = PointsRenderer(
            rasterizer=canonical_rasterizer, compositor=AlphaCompositor()
        )
        canonical_renderer.to(device)
        rendered_features = rearrange(
            canonical_renderer(point_cloud, eps=1e-5), "b h w c -> b c h w"
        )
    ...
    def setup_given_cameras(self, batch):
        ...
        src_camera_Rt = geometry.construct_Rt_matrix(
            batch["rotation1"], batch["position1"]
        )
        dst_camera_Rt = geometry.construct_Rt_matrix(
            batch["rotation2"], batch["position2"]
        )
        return src_camera_K_inv, dst_camera_K_inv, src_camera_Rt, dst_camera_Rt

    def warp(
        self, features_src, depth_src, src_camera_K_inv, dst_camera_K_inv, Rt_src_to_dst
    ):
        ...
        return self.render_features_from_points(
            src_points_in_dst_camera_coords, features_src
        )
    ...
    def render_warped_images_from_ground_truth_data(self, batch):
        ...
        Rt_1_to_2 = torch.einsum(
            "bij,bjk->bik", torch.linalg.inv(image2_camera_Rt), image1_camera_Rt
        )
        Rt_2_to_1 = torch.einsum(
            "bij,bjk->bik", torch.linalg.inv(image1_camera_Rt), image2_camera_Rt
        )
        image1_warped_onto_image2 = self.warp(
            batch["image1"],
            batch["depth_1"],
            image1_camera_K_inv,
            image2_camera_K_inv,
        ...
        image2_warped_onto_image1 = self.warp(
            batch["image2"],
            batch["depth_2"],
            image2_camera_K_inv,
            image1_camera_K_inv,
