...
class TripletSkipAllKPConv(UnwrappedUnetBasedModel):
    ...
    def _init_from_compact_format(self, opt, model_type, dataset, modules_lib):
        ...
        in lists
        """
        self.down_modules_mono = nn.ModuleList()
        self.down_modules_diff = nn.ModuleList()
        self.inner_modules = nn.ModuleList()
        self.up_modules = nn.ModuleList()

        self.save_sampling_id_1 = opt.down_conv_mono.get('save_sampling_id')
        self.save_sampling_id_2 = opt.down_conv_diff.get('save_sampling_id')

        # Factory for creating up and down modules
        factory_module_cls = self._get_factory(model_type, modules_lib)
        down_conv_cls_name_mono = opt.down_conv_mono.module_name
        down_conv_cls_name_diff = opt.down_conv_diff.module_name
        up_conv_cls_name = opt.up_conv.module_name if opt.get('up_conv') is not None else None
        self._factory_module = factory_module_cls(
            down_conv_cls_name_mono, down_conv_cls_name_diff, up_conv_cls_name, modules_lib
        ...
        # Down modules
        for i in range(len(opt.down_conv_mono.down_conv_nn)):
            args = self._fetch_arguments(opt.down_conv_mono, i, "DOWN_1")
            conv_cls = self._get_from_kwargs(args, "conv_cls")
            down_module = conv_cls(**args)
            self._save_sampling_and_search(down_module)
            self.down_modules_mono.append(down_module)
        for i in range(len(opt.down_conv_diff.down_conv_nn)):
            args = self._fetch_arguments(opt.down_conv_diff, i, "DOWN_2")
            conv_cls = self._get_from_kwargs(args, "conv_cls")
            down_module = conv_cls(**args)
            self._save_sampling_and_search(down_module)
            self.down_modules_diff.append(down_module)
    ...
    def forward(self, data) -> Any:
        ...
        for i in range(len(self.down_modules_mono) - 1):
            ...
            if datadiff is not None:
                nn_list = knn(diff.pos, datadiff.pos, 1, diff.batch, datadiff.batch)
                datadiff.x = torch.cat((datadiff.x, diff.x[nn_list[1, :], :]), axis=1)
            else:
                datadiff = diff.clone()
        ...
        if not isinstance(self.inner_modules[0], Identity):
            stack_down.append(datadiff)
            datadiff = self.inner_modules[0](datadiff)
            innermost = True
