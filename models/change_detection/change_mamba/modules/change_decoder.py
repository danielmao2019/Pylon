...
from models.change_detection.change_mamba.modules.res_block import ResBlock
from models.change_detection.change_mamba.modules.vmamba import Permute, VSSBlock


class ChangeDecoder(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.st_block_41 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-1] * 2, out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
                drop_path=0.1,
            ...
            Permute(0, 3, 1, 2) if not channel_first else torch.nn.Identity(),
        )
        self.st_block_42 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-1], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_43 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-1], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
        ...
        self.st_block_31 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-2] * 2, out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_32 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-2], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_33 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-2], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
        ...
        self.st_block_21 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-3] * 2, out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_22 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-3], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_23 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-3], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
        ...
        self.st_block_11 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-4] * 2, out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_12 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-4], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
            VSSBlock(
                hidden_dim=128,
        ...
        self.st_block_13 = torch.nn.Sequential(
            torch.nn.Conv2d(
                kernel_size=1, in_channels=encoder_dims[-4], out_channels=128
            ),
            Permute(0, 2, 3, 1) if not channel_first else torch.nn.Identity(),
        ...
        # Fuse layer
        self.fuse_layer_4 = torch.nn.Sequential(
            torch.nn.Conv2d(kernel_size=1, in_channels=128 * 5, out_channels=128),
            torch.nn.BatchNorm2d(128),
            torch.nn.ReLU(),
        )
        self.fuse_layer_3 = torch.nn.Sequential(
            torch.nn.Conv2d(kernel_size=1, in_channels=128 * 5, out_channels=128),
            torch.nn.BatchNorm2d(128),
            torch.nn.ReLU(),
        )
        self.fuse_layer_2 = torch.nn.Sequential(
            torch.nn.Conv2d(kernel_size=1, in_channels=128 * 5, out_channels=128),
            torch.nn.BatchNorm2d(128),
            torch.nn.ReLU(),
        )
        self.fuse_layer_1 = torch.nn.Sequential(
            torch.nn.Conv2d(kernel_size=1, in_channels=128 * 5, out_channels=128),
            torch.nn.BatchNorm2d(128),
            torch.nn.ReLU(),
        )
