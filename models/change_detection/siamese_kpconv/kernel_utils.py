...
def kernel_point_optimization_debug(
    ...
    for k in range(num_kernels):
        ...
        if num_free > 0:
            ...
            free_points = (
                np.random.rand(num_free, dimension) * 2 * init_radius - init_radius
            )
            if fixed == "center":
        ...
        for iter_count in range(n_iter):
            ...
            for i in range(num_free):
                ...
                if np.any(other_indices):
                    ...
                    direction = vectors / (
                        dist[:, np.newaxis] + 1e-6
                    )  # Small epsilon for stability
                    repulsion = (
                        direction.T * repulsion_strength / (dist + 1e-6)
                    ).T  # Small epsilon for stability
            ...
            if gradient_norm > 1e-6:  # Only apply if gradient is significant
                ...
                if fixed == "center":
                    kernel_points_new[1:, :] = (
                        kernel_points[1:, :] + gradient * min_dist
                    )
                elif fixed == "verticals":
                    for i in range(num_free):
                        point_idx = i + 1
                        if vertical_indices[point_idx] > 0:
                            axis = vertical_indices[point_idx] - 1
                            kernel_points_new[point_idx, axis] = (
                                kernel_points[point_idx, axis]
                                + gradient[i, axis] * min_dist
                            )
                        else:
                            kernel_points_new[point_idx, :] = (
                                kernel_points[point_idx, :] + gradient[i, :] * min_dist
                            )
                else:
