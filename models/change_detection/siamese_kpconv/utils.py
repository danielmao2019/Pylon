...
def add_ones(pos, x, add_one=True, expected_channels=None):
    ...
    if add_one:
        ...
        if (
            ...
            and x is not None
            and x.size(1) >= expected_channels
        ):
            return x
...
def gather(x, idx):
    ...
    batch_size, num_points, num_dims = (
        x.shape if len(x.shape) == 3 else (1, x.shape[0], x.shape[1])
    )
    idx_flattened = (
        idx + torch.arange(batch_size, device=device).view(-1, 1, 1) * num_points
    )
    return x.reshape(batch_size * num_points, num_dims)[
        idx_flattened.reshape(-1)
    ].reshape(idx.shape + (num_dims,))
...
def knn(x, y, k, batch_x=None, batch_y=None):
    ...
    for b in torch.unique(batch_x):
        ...
        cols = torch.nonzero(y_mask, as_tuple=True)[0][nn_idx.reshape(-1)].reshape(
            rows.size(0), -1
        )
