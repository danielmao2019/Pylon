...
class I3PEModel(torch.nn.Module):
    ...
    def __init__(
        ...
        if Backbone == 'ResNet50':
            self.encoder = ResNet50(
                BatchNorm=BatchNorm, pretrained=pretrained, output_stride=output_stride
            )

            self.fuse_layer_4 = torch.nn.Conv2d(
                kernel_size=1, in_channels=4096, out_channels=128
            )
            self.fuse_layer_3 = torch.nn.Conv2d(
                kernel_size=1, in_channels=2048, out_channels=128
            )
            self.fuse_layer_2 = torch.nn.Conv2d(
                kernel_size=1, in_channels=1024, out_channels=128
            )
            self.fuse_layer_1 = torch.nn.Conv2d(
                kernel_size=1, in_channels=512, out_channels=128
            )

            self.smooth_layer_3 = torch.nn.Conv2d(
                kernel_size=3, in_channels=128, out_channels=128, padding=1
            )
            self.smooth_layer_2 = torch.nn.Conv2d(
                kernel_size=3, in_channels=128, out_channels=128, padding=1
            )
            self.smooth_layer_1 = torch.nn.Conv2d(
                kernel_size=3, in_channels=128, out_channels=128, padding=1
            )

            self.main_clf_1 = torch.nn.Conv2d(
            ...
            self.fuse_layer_2 = torch.nn.Conv2d(
                kernel_size=1, in_channels=256, out_channels=128
            )
            self.fuse_layer_1 = torch.nn.Conv2d(
                kernel_size=1, in_channels=128, out_channels=128
            )
    ...
    def _forward(self, img_1: torch.Tensor, img_2: torch.Tensor) -> torch.Tensor:
        (
            _,
            pre_low_level_feat_1,
            pre_low_level_feat_2,
            pre_low_level_feat_3,
            pre_output,
        ) = self.encoder(img_1)
        (
            _,
            post_low_level_feat_1,
            post_low_level_feat_2,
            post_low_level_feat_3,
            post_output,
        ) = self.encoder(img_2)
        ...
        output_1 = torch.nn.functional.interpolate(
            output_1, size=img_1.size()[-2:], mode='bilinear'
        )
        return output_1
