...
class Conv2d_BN(nn.Sequential):
    def __init__(
        ...
        # self.bias = bias
        self.add_module(
            'c', nn.Conv2d(a, b, ks, stride, pad, dilation, groups, bias=bias)
        )
...
class Conv1d_BN(nn.Sequential):
    def __init__(
        ...
        # self.bias = bias
        self.add_module(
            'c', nn.Conv1d(a, b, ks, stride, pad, dilation, groups, bias=bias)
        )
        bn = build_norm_layer(norm_cfg, b)[1]
        nn.init.constant_(bn.weight, bn_weight_init)
        nn.init.constant_(bn.bias, 0)
...
class DEACA_attention(torch.nn.Module):
    def __init__(
        ...
        self.proj = torch.nn.Sequential(
            activation(), Conv1d_BN(dim, dim, bn_weight_init=0)
        )
    ...
    def forward(self, q_row, q_col, k_row, k_col, v):
        ...
        q_row = (
            q_row.contiguous()
            .view(tgt_len, B * self.num_heads, self.head_dim)
            .transpose(0, 1)
        )  # [32,5,32]
        q_col = (
            q_col.contiguous()
        ...
        v = (
            v.contiguous()
            .permute(1, 2, 0, 3)
            .reshape(H, W, B * self.num_heads, self.head_dim)
            .permute(2, 0, 1, 3)
        )  # [32,128,128,32]
        v_avg = (
            self.sigmoid(self.pwconv(self.avg_pool(v.permute(0, 3, 1, 2))))
            .squeeze(-1)
            .permute(0, 2, 1)
        )  # [32, 1, 32]
        ...
        attn_row = (
            torch.matmul(q_row, k_row.transpose(1, 2)) * self.scaling
        )  # [32,5,128]
        attn_row = attn_row.softmax(dim=-1)
        xx_row = torch.matmul(attn_row, v_row)  # [32,5,32]
        xx_row = self.proj_encode_row(
            xx_row.permute(0, 2, 1).reshape(B, self.dim, tgt_len)
        )  # [4,256,5]
        ...
        xx_col = self.proj_encode_column(
            xx_col.permute(0, 2, 1).reshape(B, self.dim, tgt_len)
        )  # [4,256,5]
...
class DEACA_attention_v3(torch.nn.Module):
    ...
    def forward(self, query_row, query_col, key_row, key_col, value):
        ...
        v_avg = (
            self.sigmoid(self.conv(self.avg_pool(v.permute(0, 3, 1, 2))))
            .squeeze(-1)
            .permute(0, 2, 1)
        )  # [32, 1, 32]
        ...
        xx_row = torch.matmul(attn_row, v_row)  # [32,5,32]
        xx_row = self.proj_encode_row(
            xx_row.permute(0, 2, 1).reshape(B, self.embed_dim, tgt_len).permute(2, 0, 1)
        )  # [4,256,5]

        attn_col = torch.matmul(q_col, k_col.transpose(1, 2)) * self.scaling
        attn_col = attn_col.softmax(dim=-1)
        xx_col = torch.matmul(attn_col, v_col)  # [32,5,32]
        xx_col = self.proj_encode_col(
            xx_col.permute(0, 2, 1).reshape(B, self.embed_dim, tgt_len).permute(2, 0, 1)
        )  # [4,256,5]
...
class rcda_rebuild(nn.Module):
    ...
    def forward(self, query_row, query_col, key_row, key_col, value):
        v_avg = (
            self.sigmoid(
                self.lin(
                    self.avg_pool(value.permute(0, 3, 1, 2)).squeeze(-1).squeeze(-1)
                )
            )
            .unsqueeze(-1)
            .permute(2, 0, 1)
        ...
        q_row = (
            q_row.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        )
        q_col = (
            q_col.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        )
