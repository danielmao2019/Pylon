...
def ms_deform_attn_core_pytorch(
    ...
    )  # 把范围从[0,1]转换到[-1,1], F.grid_sample要求grid的范围是[-1,1]
    sampling_value_list = []
    for lid_, (H_, W_) in enumerate(value_spatial_shapes):
        # N_, H_*W_, M_, D_ -> N_, H_*W_, M_*D_ -> N_, M_*D_, H_*W_ -> N_*M_, H_, W_
        value_l_ = (
            value_list[lid_].flatten(2).transpose(1, 2).reshape(N_ * M_, Dim, H_, W_)
        )  # eg. [bs * 8, 32, 28, 28, 28]
        # N_, Lq_, M_, P_, 3 -> N_, M_, Lq_, P_, 3 -> N_*M_, Lq_, P_, 3
        sampling_grid_l_ = sampling_grids[:, :, :, lid_]
        sampling_grid_l_ = sampling_grid_l_.transpose(1, 2).flatten(
            0, 1
        )  # eg. [bs * 8, 1045, 3, 3]
        # N_*M_, D_, Lq_, P_
