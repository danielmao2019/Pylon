...
from models.change_detection.cdmaskformer.pixel_decoder.ms_deform_attn import (
    MSDeformAttn,
)
from models.change_detection.cdmaskformer.transformer_decoder.position_encoding import (
    PositionEmbeddingSine,
)
from models.change_detection.cdmaskformer.transformer_decoder.transformer import (
    _get_activation_fn,
    _get_clones,
)
...
class MSDeformAttnPixelDecoder(nn.Module):
    def __init__(
        ...
        self.in_features = [
            k for k, v in input_shape.items()
        ]  # starting from "res3" to "res5"
        self.feature_channels = [
            v.channel for k, v in input_shape.items()
        ]  # eg. [16, 64, 128, 256]

        # this is the input shape of transformer encoder (could use less features than pixel decoder
        self.transformer_in_features = [
            k for k, v in transformer_input_shape.items()
        ]  # starting from "res3" to "res5"
        transformer_in_channels = [
            v.channel for k, v in transformer_input_shape.items()
        ]  # eg. [64, 128, 256]
        self.transformer_feature_strides = [
            v.stride for k, v in transformer_input_shape.items()
...
class MSDeformAttnPixelDecoder4ScalesFASeg(nn.Module):
    def __init__(
        ...
        self.transformer_num_feature_levels = len(self.transformer_in_features)
        # self.pos_linear = len(self.transformer_in_features)
        ...
        # For "res2", positional encoding generator
        pos_linear_list.append(
            nn.Conv2d(
                256, 256, kernel_size=3, stride=1, padding=1, bias=True, groups=256
            )
        )
        ...
        self.maskformer_num_feature_levels = 4  # Here we use 4 scales
        self.common_stride = common_stride
