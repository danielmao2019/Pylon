...
class Sea_Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        ## squeeze row
        qrow = (
            self.pos_emb_rowq(q.mean(-1))
            .reshape(B, self.num_heads, -1, H)
            .permute(0, 1, 3, 2)
        )
        ...
        ## squeeze column
        qcolumn = (
            self.pos_emb_columnq(q.mean(-2))
            .reshape(B, self.num_heads, -1, W)
            .permute(0, 1, 3, 2)
        )
...
def SeaFormer_L(
    pretrained=False,
    weights='./models/change_detection/cdmaskformer/SeaFormer_L_cls_79.9.pth.tar',
    **kwargs,
):
    model_cfgs = dict(
        cfg1=[
