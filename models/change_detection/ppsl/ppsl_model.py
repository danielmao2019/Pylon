...
def Conv3x3ReLUBNs(in_channels, inner_channels, num_convs):
...
class metric_attention(nn.Module):
    def __init__(self, in_channels=256, inner_channels=256, num_convs=2):
        ...
        self.compare = Conv3x3ReLUBNs(
            in_channels=in_channels, inner_channels=inner_channels, num_convs=num_convs
        )
    ...
    def forward(self, x1, x2):
        ...
        metric = torch.norm(
            F.normalize(self.squeeze(x1), dim=1, eps=1e-6)
...
class featureFused(nn.Module):
    ...
    def forward(self, input):
        ...
        x3 = nn.functional.interpolate(
            input[1], size=(h, w), mode='bilinear', align_corners=True
        )
        x4 = nn.functional.interpolate(
            input[2], size=(h, w), mode='bilinear', align_corners=True
        )
        x5 = nn.functional.interpolate(
            input[3], size=(h, w), mode='bilinear', align_corners=True
        )
...
class PPSLModel(nn.Module):
    def __init__(self, backbone='resnet18'):
        super(PPSLModel, self).__init__()
        self.backbone = changeBackbone(backbone)

        # get change feature
        self.compare_c2 = metric_attention(
            in_channels=256, inner_channels=256, num_convs=2
        )
        self.compare_c3 = metric_attention(
            in_channels=256, inner_channels=256, num_convs=2
        )
        self.compare_c4 = metric_attention(
            in_channels=256, inner_channels=256, num_convs=2
        )
        self.compare_c5 = metric_attention(
            in_channels=256, inner_channels=256, num_convs=2
        )
        ...
        self.head = nn.Sequential(
            ...
            nn.Upsample(scale_factor=2),
            nn.Conv2d(
                in_channels=128, out_channels=2, kernel_size=3, stride=1, padding=1
            ),
            nn.Sigmoid(),
        )
