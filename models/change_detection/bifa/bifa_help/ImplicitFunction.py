...
from models.change_detection.bifa.bifa_help.implicit_help import ifa_simfpn
...
class fpn_ifa(nn.Module):
    ...
    def __init__(self, in_planes, num_classes=2, inner_planes=256, sync_bn=False, dilations=(12, 24, 36),
        ...
        if self.no_aspp:
            self.head = nn.Sequential(nn.Conv2d(256, 256, kernel_size=1), norm_layer(256), nn.ReLU(inplace=True))
        else:
        ...
        self.ifa = ifa_simfpn(ultra_pe=ultra_pe, pos_dim=pos_dim, sync_bn=sync_bn, num_classes=num_classes, local=local,
                              ...
                              num_layer=num_layer)
        self.enc1 = nn.Sequential(nn.Conv2d(32, 256, kernel_size=1), norm_layer(256), nn.ReLU(inplace=True))
        self.enc2 = nn.Sequential(nn.Conv2d(64, 256, kernel_size=1), norm_layer(256), nn.ReLU(inplace=True))
        self.enc3 = nn.Sequential(nn.Conv2d(160, 256, kernel_size=1), norm_layer(256), nn.ReLU(inplace=True))
