...
class OverlapPredator(nn.Module):
    ...
    def __init__(self, **config):
        ...
        self.proj_gnn = nn.Conv1d(
            gnn_feats_dim, gnn_feats_dim, kernel_size=1, bias=True
        )
    ...
    def forward(self, batch):
        ...
        ####################################
        # 4. decoder part
        src_feats_gnn, tgt_feats_gnn = (
            feats_gnn_norm[:len_src_c],
            feats_gnn_norm[len_src_c:],
        )
        ...
        s2 = torch.matmul(
            F.softmax(inner_products.transpose(0, 1) / temperature, dim=1), src_scores_c
        )
