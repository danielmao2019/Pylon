...
from models.point_cloud_registration.buffer.KPConv.lib.benchmark import (
    read_trajectory,
    write_trajectory,
)
...
def fmr_wrt_distance(data, split, inlier_ratio_threshold=0.05):
    ...
    for distance_threshold in range(1, 21):
        ...
        for ele in split:
            fmr += (
                np.array(inlier_ratios[ele[0] : ele[1]]) > inlier_ratio_threshold
            ).mean()
        fmr /= 8
...
def write_est_trajectory(gt_folder, exp_dir, tsfm_est):
    ...
    for scene_name in scene_names:
        gt_pairs, gt_traj = read_trajectory(
            os.path.join(gt_folder, scene_name, 'gt.log')
        )
        est_traj = []
        ...
        write_trajectory(
            np.array(est_traj), gt_pairs, os.path.join(c_directory, 'est.log')
        )
...
def get_inlier_ratio(
    src_pcd, tgt_pcd, src_feat, tgt_feat, rot, trans, inlier_distance_threshold=0.1
):
    """
