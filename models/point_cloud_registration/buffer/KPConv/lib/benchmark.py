...
def read_trajectory_info(filename, dim=6):
    ...
    for i in range(n_pairs):
        frame_idx0, frame_idx1, n_frame = [
            int(item) for item in contents[i * 7].strip().split()
        ]
        info_matrix = np.concatenate(
            [
                np.fromstring(item, sep='\t').reshape(1, -1)
                for item in contents[i * 7 + 1 : i * 7 + 7]
            ],
            axis=0,
        )
    ...
    with open(filename, 'w') as f:
        for idx in range(traj.shape[0]):
            ...
            if metadata[idx][2]:
                ...
                f.write(
                    '\n'.join(
                        '\t'.join(map('{0:.12f}'.format, p[i])) for i in range(dim)
                    )
                )
                f.write('\n')
...
def evaluate_registration(
    num_fragment, result, result_pairs, gt_pairs, gt, gt_info, err2=0.2
):
    """
    ...
    for idx in range(result_pairs.shape[0]):
        ...
        if gt_mask[i, j] > 0:
            n_res += 1
            gt_idx = gt_mask[i, j]
            p = computeTransformationErr(
                np.linalg.inv(gt[gt_idx, :, :]) @ pose, gt_info[gt_idx, :, :]
            )
            if p <= err2:
                good += 1
                flags.append(0)
...
def benchmark(est_folder, gt_folder):
    ...
    with open(f'{est_folder}/result', 'w') as f:
        ...
        for idx, scene in enumerate(scene_names):
            ...
            n_fragments, gt_traj_cov = read_trajectory_info(
                os.path.join(scene, "gt.info")
            )

            # estimated info
            est_pairs, est_traj = read_trajectory(
                os.path.join(est_folder, scenes[idx], 'est.log')
            )
            ...
            re = (
                rotation_error(
                    torch.from_numpy(ext_gt_traj[:, 0:3, 0:3]),
                    torch.from_numpy(est_traj[:, 0:3, 0:3]),
                )
                .cpu()
            ...
            te = (
                translation_error(
                    torch.from_numpy(ext_gt_traj[:, 0:3, 3:4]),
                    torch.from_numpy(est_traj[:, 0:3, 3:4]),
                )
                .cpu()
                .numpy()[np.array(c_flag) == 0]
            )
        ...
        weighted_precision = (np.array(n_valids) * np.array(precision)).sum() / np.sum(
            n_valids
        )
