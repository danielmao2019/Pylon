...
from models.point_cloud_registration.buffer.KPConv.lib.benchmark_utils import (
    get_angle_deviation,
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)):  # loop through this epoch
                ...
                torch.save(
                    data, f'{self.snapshot_dir}/{self.config.benchmark}/{idx}.pth'
                )
        ...
        with torch.no_grad():
            for _ in tqdm(range(num_iter)):  # loop through this epoch
                ...
                src_overlap, tgt_overlap = (
                    scores_overlap[:len_src],
                    scores_overlap[len_src:],
                )
                src_saliency, tgt_saliency = (
                    scores_saliency[:len_src],
                    scores_saliency[len_src:],
                )
                ...
                # ##### visulization test ########
                # src_pcd = (torch.matmul(c_rot, src_pcd.transpose(0, 1)) + c_trans).transpose(0, 1)
                # import open3d as o3d
                # pcd0 = o3d.geometry.PointCloud()
                # pcd0.points = o3d.utility.Vector3dVector(src_pcd.detach().cpu().numpy())
                # pcd0.paint_uniform_color([0.2, 0.3, 0.4])
                # pcd1 = o3d.geometry.PointCloud()
                # pcd1.points = o3d.utility.Vector3dVector(tgt_pcd.detach().cpu().numpy())
                # pcd1.paint_uniform_color([0.7, 0.8, 0.9])
...
def compute_metrics(data, pred_transforms):
    ...
    with torch.no_grad():
        ...
        src_clean = se3.transform(
            se3.concatenate(pred_transforms, se3.inverse(gt_transforms)), points_raw
        )
...
def print_metrics(logger, summary_metrics, losses_by_iteration=None, title='Metrics'):
    ...
    logger.info(
        'DeepCP metrics:{:.4f}(rot-rmse) | {:.4f}(rot-mae) | {:.4g}(trans-rmse) | {:.4g}(trans-mae)'.format(
    ...
    logger.info(
        'Rotation error {:.4f}(deg, mean) | {:.4f}(deg, rmse)'.format(
            summary_metrics['err_r_deg_mean'], summary_metrics['err_r_deg_rmse']
        )
    )
    logger.info(
        'Translation error {:.4g}(mean) | {:.4g}(rmse)'.format(
            summary_metrics['err_t_mean'], summary_metrics['err_t_rmse']
        )
    )
    logger.info(
        'Chamfer error: {:.7f}(mean-sq)'.format(summary_metrics['chamfer_dist'])
    )
...
class ModelnetTester(Trainer):
    ...
    def test(self):
        print('Start to evaluate on test datasets...')
        _logger, _log_path = prepare_logger(
            self.config, log_path=os.path.join(self.snapshot_dir, 'results')
        )
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)):  # loop through this epoch
                ...
                try:
                    ...
                    rot_trace = (
                        inputs['sample']['transform_gt'][:, 0, 0]
                        + inputs['sample']['transform_gt'][:, 1, 1]
                        + inputs['sample']['transform_gt'][:, 2, 2]
                    )
                    rotdeg = (
                        torch.acos(
                            torch.clamp(0.5 * (rot_trace - 1), min=-1.0, max=1.0)
                        )
                        * 180.0
                        / np.pi
                    )
                    ...
                    if src_pcd.size(0) > n_points:
                        ...
                        idx = np.random.choice(
                            idx, size=n_points, replace=False, p=probs
                        )
                        src_pcd, src_feats = src_pcd[idx], src_feats[idx]
                    if tgt_pcd.size(0) > n_points:
                        idx = np.arange(tgt_pcd.size(0))
        ...
        _logger.info(
            (
                'Rotation range in data: {}(avg), {}(max)'.format(
                    np.mean(total_rotation), np.max(total_rotation)
                )
            )
        )

        pred_transforms = torch.from_numpy(np.array(pred_transforms)).float()[
            :, None, :, :
        ]
