...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x, lrf=None):
        ...
        deformed_K_points = self.kernel_points.to(neighbors.device)

        # rotate local point sets via rods rotation matrix
        if lrf is not None:
            neighbors = torch.matmul(neighbors, lrf)
            deformed_K_points = torch.matmul(deformed_K_points, lrf).unsqueeze(1)
...
class detector(nn.Module):
    ...
    def forward(self, pts, stack_length, neighb_inds, x):
        score = self.linear(x)
        len_src, len_tgt = stack_length[0], stack_length[1]

        # separate src and tgt
        src_pts, tgt_pts = pts[:len_src], pts[len_src:]
        src_feat, tgt_feat = x[:len_src], x[len_src:]
...
class SimpleBlock(nn.Module):
    ...
    def forward(self, x, batch):
        ...
        if 'strided' in self.block_name:
            ...
            # q_normals = batch['normals'][self.layer_ind + 1]
            # q_rods_rotat = RodsRotatFormula(q_normals.float(), torch.FloatTensor([0, 0, 1]).expand_as(q_normals))
        else:
            q_pts = batch['points'][self.layer_ind]
            s_pts = batch['points'][self.layer_ind]
            neighb_inds = batch['neighbors'][self.layer_ind]
            # q_normals = batch['normals'][self.layer_ind]
            # q_rods_rotat = RodsRotatFormula(q_normals.float(), torch.FloatTensor([0, 0, 1]).expand_as(q_normals))

        x = self.KPConv(q_pts, s_pts, neighb_inds, x, batch['lrf'])
