...
class VNStdFeature(nn.Module):
    def __init__(
        ...
        if normalize_frame:
            self.vn_lin = nn.Linear(in_channels // 2, 2, bias=False)
        else:
            self.vn_lin = nn.Linear(in_channels // 2, 3, bias=False)

    def forward(self, x, out_lrf=False):
        '''
        x: point features of shape [B, N_feat, 3, N_samples, ...]
        '''
        assert self.normalize_frame == out_lrf
