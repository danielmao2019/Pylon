...
def point_to_node_partition(
    ...
    node_masks = torch.zeros(
        nodes.shape[0], dtype=torch.bool, device=sq_dist_mat.device
    )  # (M,)
    ...
    node_indices = (
        torch.arange(nodes.shape[0], device=node_knn_node_indices.device)
        .unsqueeze(1)
        .expand(-1, point_limit)
    )  # (M, K)
    ...
    if return_count:
        ...
        node_sizes = torch.zeros(
            nodes.shape[0], dtype=torch.long, device=point_to_node.device
        )  # (M,)
