...
from models.point_cloud_registration.geotransformer.lrpe_transformer import (
    LRPETransformerLayer,
)
from models.point_cloud_registration.geotransformer.pe_transformer import (
    PETransformerLayer,
)
from models.point_cloud_registration.geotransformer.rpe_transformer import (
    RPETransformerLayer,
)
from models.point_cloud_registration.geotransformer.vanilla_transformer import (
    TransformerLayer,
)
...
class PEConditionalTransformer(nn.Module):
    ...
    def forward(
        self, feats0, feats1, embeddings0, embeddings1, masks0=None, masks1=None
    ):
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](
                    feats0, feats0, embeddings0, embeddings0, memory_masks=masks0
                )
                feats1, scores1 = self.layers[i](
                    feats1, feats1, embeddings1, embeddings1, memory_masks=masks1
                )
            else:
                feats0, scores0 = self.layers[i](feats0, feats1, memory_masks=masks1)
                feats1, scores1 = self.layers[i](feats1, feats0, memory_masks=masks0)
...
class RPEConditionalTransformer(nn.Module):
    ...
    def forward(
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](
                    feats0, feats0, embeddings0, memory_masks=masks0
                )
                feats1, scores1 = self.layers[i](
                    feats1, feats1, embeddings1, memory_masks=masks1
                )
            else:
                if self.parallel:
                    new_feats0, scores0 = self.layers[i](
                        feats0, feats1, memory_masks=masks1
                    )
                    new_feats1, scores1 = self.layers[i](
                        feats1, feats0, memory_masks=masks0
                    )
                    feats0 = new_feats0
                    feats1 = new_feats1
                else:
...
class LRPEConditionalTransformer(nn.Module):
    ...
    def forward(
        self, feats0, feats1, emb_indices0, emb_indices1, masks0=None, masks1=None
    ):
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](
                    feats0, feats0, emb_indices0, memory_masks=masks0
                )
                feats1, scores1 = self.layers[i](
                    feats1, feats1, emb_indices1, memory_masks=masks1
                )
            else:
                feats0, scores0 = self.layers[i](feats0, feats1, memory_masks=masks1)
                feats1, scores1 = self.layers[i](feats1, feats0, memory_masks=masks0)
