...
from models.point_cloud_registration.parenet.pareconv.modules.layers import (
    VNLeakyReLU,
    VNLinear,
    VNLinearLeakyReLU,
    VNStdFeature,
...
class CorrelationNet(nn.Module):
    ...
    def __init__(
        self, in_channel, out_channel, hidden_unit=[8, 8], last_bn=False, temp=1
    ):
        ...
        for i in range(
            1, len(hidden_unit)
        ):  # from 1st hidden to next hidden to last hidden
    ...
    def forward(self, xyz):
        ...
        scores = torch.norm(
            scores, p=2, dim=2
        )  # transform rotation equivairant feats into rotation invariant feats
...
class PARE_Conv_Block(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, share_nonlinearity=False):
        ...
        self.score_net = CorrelationNet(
            in_channel=3, out_channel=self.kernel_size, hidden_unit=[self.kernel_size]
        )

        in_dim = in_dim + 2  # 1 + 2: [xyz, mean, cross]
        tensor1 = nn.init.kaiming_normal_(
            torch.empty(self.kernel_size, in_dim, out_dim // 2)
        ).contiguous()
        tensor1 = tensor1.permute(1, 0, 2).reshape(
            in_dim, self.kernel_size * out_dim // 2
        )
    ...
    def forward(self, q_pts, s_pts, s_feats, neighbor_indices):
        ...
        pts = (
            (s_pts[neighbor_indices] - q_pts[:, None]).unsqueeze(1).permute(0, 1, 3, 2)
        )  # [N, 1, 3, K]
        centers = pts.mean(-1, keepdim=True).repeat(1, 1, 1, K)
        cross = torch.cross(pts, centers, dim=2)
        local_feats = torch.cat(
            [pts, centers, cross], 1
        )  # [N, 3, 3, K] rotation equivariant spatial features
...
class PARE_Conv_Resblock(nn.Module):
    def __init__(
        ...
        if self.use_xyz:
            conv_dim += 1
        tensor1 = nn.init.kaiming_normal_(
            torch.empty(self.kernel_size, conv_dim, out_dim // 2)
        ).contiguous()
        tensor1 = tensor1.permute(1, 0, 2).reshape(
            conv_dim, self.kernel_size * out_dim // 2
        )
        self.weightbank = nn.Parameter(tensor1, requires_grad=True)

        self.relu = VNLeakyReLU(out_dim // 2, share_nonlinearity)
        self.shortcut_proj = (
            VNLinear(in_dim, out_dim) if shortcut_linear else nn.Identity()
        )
        self.unary = VNLinearLeakyReLU(out_dim // 2, out_dim)
...
class PAREConvFPN(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        feats_s4 = self.encoder4_1(
            points_list[3], points_list[2], feats_s3, subsampling_list[2]
        )
        feats_s4 = self.encoder4_2(
            points_list[3], points_list[3], feats_s4, neighbors_list[3]
        )
        feats_s4 = self.encoder4_3(
            points_list[3], points_list[3], feats_s4, neighbors_list[3]
        )
