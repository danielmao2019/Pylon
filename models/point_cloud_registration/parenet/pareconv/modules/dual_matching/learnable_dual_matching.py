...
class PointDualMatching(nn.Module):
    ...
    def forward(
        ...
        """
        m_ref_feats, m_src_feats = self.proj1(ref_node_corr_knn_feats), self.proj1(
            src_node_corr_knn_feats
        )
        ...
        padded_row_masks.masked_fill_(
            ~src_node_corr_knn_masks[:, None, :], float('-inf')
        )

        padded_col_masks = torch.zeros(size=(batch_size, num_row, num_col)).cuda()
        padded_col_masks.masked_fill_(
            ~ref_node_corr_knn_masks[:, :, None], float('-inf')
        )
        matching_scores = F.softmax(scores + padded_row_masks, -1) * F.softmax(
            scores + padded_col_masks, 1
        )
        matching_scores = (
            matching_scores
