...
class LocalGlobalRegistration(nn.Module):
    ...
    def convert_to_batch(ref_corr_points, src_corr_points, corr_scores, chunks):
        ...
        target_chunks = [
            (i * max_corr, i * max_corr + y - x) for i, (x, y) in enumerate(chunks)
        ]
        indices = torch.cat(
            [torch.arange(x, y) for x, y in target_chunks], dim=0
        ).cuda()
        indices0 = indices.unsqueeze(1).expand(
            indices.shape[0], 3
        )  # (total,) -> (total, 3)
        indices1 = (
            torch.arange(3).unsqueeze(0).expand(indices.shape[0], 3).cuda()
        )  # (3,) -> (total, 3)
    ...
    def local_to_global_registration(
        ...
        if (
            self.correspondence_limit is not None
            and global_corr_scores.shape[0] > self.correspondence_limit
        ):
        ...
        if batch_size > 0:
            # local registration
            batch_ref_corr_points, batch_src_corr_points, batch_corr_scores = (
                self.convert_to_batch(
            ...
            batch_transforms = self.procrustes(
                batch_src_corr_points, batch_ref_corr_points, batch_corr_scores
            )
