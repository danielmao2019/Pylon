...
class HypothesisProposer(nn.Module):
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        ref_indices = (
            torch.arange(ref_length)
            .cuda()
            .view(1, ref_length, 1)
            .expand(batch_size, -1, self.k)
        )  # (B, N, K)
        ref_score_mat = torch.zeros_like(score_mat)
        ref_score_mat[ref_batch_indices, ref_indices, ref_topk_indices] = (
            ref_topk_scores
        )
        ...
        src_indices = (
            torch.arange(src_length)
            .cuda()
            .view(1, 1, src_length)
            .expand(batch_size, self.k, -1)
        )  # (B, K, N)
        src_score_mat = torch.zeros_like(score_mat)
        src_score_mat[src_batch_indices, src_topk_indices, src_indices] = (
            src_topk_scores
        )
        # correspondences used to vote for hypotheses
        voter_corr_mat = torch.logical_or(
        ...
        corr_scores, corr_indices = score_mat.reshape(-1).topk(
    ...
    def recompute_correspondence_scores(
        ...
        corr_residuals = torch.linalg.norm(
            ref_corr_points - aligned_src_corr_points, dim=1
        )
    ...
    def extract_fine_transforms(
        self, ref_corr_feats, src_corr_feats, ref_corr_points, src_corr_points
    ):
        point_rotations = solve_local_rotations(src_corr_feats, ref_corr_feats)  # B 3 3
        aligned_src_points = torch.einsum(
            'bmn, bn->bm', point_rotations, src_corr_points
        )
        t = ref_corr_points - aligned_src_points
        transforms = (
            torch.eye(4, device=ref_corr_feats.device)
            .unsqueeze(0)
    ...
    def feature_based_hypothesis_proposer(
        self,
        ref_knn_points,
        src_knn_points,
        ref_knn_feats,
        ...
        ref_corr_feats, src_corr_feats = (
            ref_knn_feats[batch_indices, ref_indices],
            src_knn_feats[batch_indices, src_indices],
        )
        # build verification set
        batch_v_indices, ref_v_indices, src_v_indices = torch.nonzero(
            voter_corr_mat, as_tuple=True
        )
        ...
        batch_aligned_src_corr_points = apply_transform(
            src_corr_points.unsqueeze(0), transformation_hypotheses
        )
        batch_corr_residuals = torch.linalg.norm(
            ref_corr_points.unsqueeze(0) - batch_aligned_src_corr_points, dim=2
        )
        batch_inlier_masks = torch.lt(
            batch_corr_residuals, self.acceptance_radius
        )  # (P, N)
        ...
        # global refinement
        estimated_transform = self.procrustes(
            src_corr_points, ref_corr_points, cur_corr_scores
        )
    ...
    def forward(
        ...
        corr_mat, voter_corr_mat = self.compute_correspondence_matrix(
