...
def random_scale_shift_points(
    points, low=2.0 / 3.0, high=3.0 / 2.0, shift=0.2, normals=None
):
...
def random_jitter_points(points, scale, noise_magnitude=0.05):
    ...
    noises = np.clip(
        np.random.normal(scale=scale, size=points.shape),
        a_min=-noise_magnitude,
        a_max=noise_magnitude,
    )
...
def random_jitter_features(features, mu=0, sigma=0.01):
    ...
    if random.random() < 0.95:
        features = features + np.random.normal(mu, sigma, features.shape).astype(
            np.float32
        )
    return features
...
def random_crop_point_cloud_with_plane(
    points, p_normal=None, keep_ratio=0.7, normals=None
):
...
def random_sample_viewpoint(limit=500):
    ...
    return np.random.rand(3) + np.array([limit, limit, limit]) * np.random.choice(
        [1.0, -1.0], size=3
    )


def random_crop_point_cloud_with_point(
    points, viewpoint=None, keep_ratio=0.7, normals=None
):
