...
def setup_states(
    ...
    **kwargs: Any,
    ...
    """Initialize default modality state entries for LapisGS scenes."""
    assert scene_model_cls is not None
    container = _get_model_store_container(app)

    for field, default in (
        ('lapis_selected_layers_rgb', None),
        ('lapis_selected_layers_density', None),
        ('lapis_debugger_enabled', False),
    ):
