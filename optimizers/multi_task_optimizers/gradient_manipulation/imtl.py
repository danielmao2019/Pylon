...
class IMTLOptimizer(GradientManipulationBaseOptimizer):
    ...
    def backward(
        ...
        for idx, task in enumerate(losses.keys()):
            losses[task] = (
                losses[task] * torch.exp(self.scale_params[idx])
    ...
    def _gradient_manipulation_(
        ...
        D = [
            grads_list[random_idx] - grads_list[idx]
            for idx in range(self.num_tasks)
            if idx != random_idx
        ]
        U = [
            grads_list[random_idx] / grad_norms[random_idx]
            - grads_list[idx] / grad_norms[idx]
            for idx in range(self.num_tasks)
            if idx != random_idx
        ]
        ...
        alpha = torch.matmul(
            torch.tensor([torch.dot(grads_list[random_idx], u) for u in U])
            .reshape((1, self.num_tasks - 1))
            .to(device),
            inverse,
        )
        ...
        assert (
            len(alpha) == self.num_tasks and sum(alpha) == 1
        ), f"{alpha=}, {self.num_tasks=}, {sum(alpha)=}"
