...
class MTLOptimizerTestModel(torch.nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.heads = torch.nn.ModuleDict(
            {name: torch.nn.Linear(2, 2) for name in LABEL_NAMES}
        )
...
def test_mtl_optimizer(wrt_rep: bool) -> None:
    ...
    labels = {
        name: torch.randn(size=(1, 2), dtype=torch.float32, device='cuda')
    ...
    optimizer = MTLOptimizer(
        optimizer_config=core_optimizer_config,
        losses=losses,
    ...
    assert torch.equal(
        torch.cat(
            [torch.tensor(s, dtype=torch.int64) for s in optimizer.shared_params_shapes]
        ),
