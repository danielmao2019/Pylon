...
class GeoTransformerMetric(SingleTaskMetric):
    ...
    @torch.no_grad()
    def evaluate_coarse(
        self, y_pred: Dict[str, torch.Tensor], idx: int
    ) -> torch.Tensor:
        ...
        point_inlier_ratio = PointInlierRatio(use_buffer=False)(
            y_pred={
                ...
                'correspondences': torch.stack(
                    [y_pred['src_node_corr_indices'], y_pred['ref_node_corr_indices']],
                    dim=1,
                ),
            },
            y_true={
        ...
        assert point_inlier_ratio.keys() == {
            'point_inlier_ratio'
        }, f"{point_inlier_ratio.keys()=}"
        return point_inlier_ratio['point_inlier_ratio']

    @torch.no_grad()
    def evaluate_fine(
        self, y_pred: Dict[str, torch.Tensor], y_true: Dict[str, torch.Tensor], idx: int
    ) -> torch.Tensor:
