...
class PointInlierRatio(SingleTaskMetric):
    ...
    DIRECTIONS = {'point_inlier_ratio': +1}  # Higher is better
    ...
    def __call__(self, datapoint: Dict[str, Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        assert (
            'src_pc' in outputs and 'tgt_pc' in outputs
        ), f"Expected src_pc and tgt_pc in outputs, got {outputs.keys()}"
        ...
        assert (
            'correspondences' in outputs
        ), f"Expected correspondences in outputs, got {outputs.keys()}"
        pred_correspondences = outputs['correspondences']
        assert isinstance(
            pred_correspondences, torch.Tensor
        ), f"Expected torch.Tensor for pred_correspondences, got {type(pred_correspondences)}"
        assert (
        ...
        assert (
            'correspondences' in labels
        ), f"Expected correspondences in labels, got {labels.keys()}"
        gt_correspondences = labels['correspondences']
        assert isinstance(
            gt_correspondences, torch.Tensor
        ), f"Expected torch.Tensor for gt_correspondences, got {type(gt_correspondences)}"
        assert (
        ...
        point_inlier_ratio = corr_map[
            pred_correspondences[:, 0], pred_correspondences[:, 1]
        ].mean()
