...
class PARENetMetric(SingleTaskMetric):
    ...
    def __call__(self, datapoint: Dict[str, Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        if estimated_transform.ndim == 3:
            assert (
                estimated_transform.shape[0] == 1
            ), f"Expected batch size 1, got {estimated_transform.shape[0]}"
        ...
        transform_errors = self.isotropic_error._compute_score(
        ...
        inlier_datapoint = {
            'outputs': {
                'src_pc': outputs[
                    'ref_corr_points'
                ],  # Reference points (will be transformed)
                'tgt_pc': outputs[
                    'src_corr_points'
                ],  # Source points (targets after transformation)
            },
            'labels': {'transform': labels['transform']},
        ...
        data_dict = {'transform': labels['transform']}
