...
class OverlapPredatorMetric(SingleTaskMetric):
    ...
    def get_recall(self, coords_dist, feats_dist):
        ...
        sel_dist = torch.gather(coords_dist, dim=-1, index=sel_idx[:, None])[
            pos_mask.sum(-1) > 0
        ]
    ...
    def _compute_overlap_metrics(
        ...
        cls_precision, cls_recall = self.get_cls_precision_recall(
            scores_overlap, gt_labels
        )
        return {'overlap_recall': cls_recall, 'overlap_precision': cls_precision}

    def _compute_saliency_metrics(
        self,
        scores_saliency: torch.Tensor,
        src_idx: list,
        tgt_idx: list,
        src_pcd: torch.Tensor,
        tgt_pcd: torch.Tensor,
        ...
        gt_labels = torch.cat(
            (
                (distance_1 < self.matchability_radius).float(),
        ...
        cls_precision, cls_recall = self.get_cls_precision_recall(
            scores_saliency, gt_labels
        )
        return {'saliency_recall': cls_recall, 'saliency_precision': cls_precision}

    def _compute_feature_recall(
        self,
        ...
        c_dist = torch.norm(
            src_pcd[correspondence[:, 0]] - tgt_pcd[correspondence[:, 1]], dim=1
        )
        ...
        coords_dist = torch.sqrt(
            square_distance(src_pcd[None, :, :], tgt_pcd[None, :, :]).squeeze(0)
        )
        feats_dist = torch.sqrt(
            square_distance(
                src_feats[None, :, :], tgt_feats[None, :, :], normalised=True
            )
        ).squeeze(0)
    ...
    def __call__(
        self, y_pred: Dict[str, torch.Tensor], y_true: Dict[str, Any], idx: int
        ...
        assert len(y_pred['feats_f']) == len(src_pcd) + len(
            tgt_pcd
