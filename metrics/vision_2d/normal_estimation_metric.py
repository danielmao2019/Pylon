...
class NormalEstimationMetric(SingleTaskMetric):
    ...
    def _compute_score(
        ...
        assert torch.all(
            torch.maximum(cosine_map - 1, -1 - cosine_map) < NUMERICAL_STABILITY
        ), f"{cosine_map.min()=}, {cosine_map.max()=}"
        cosine_map = torch.clamp(cosine_map, min=-1, max=+1)
        # filter by valid mask
        valid_mask = torch.linalg.norm(y_true, dim=1) != 0
        assert valid_mask.sum() > 0
        assert (
            valid_mask.shape == cosine_map.shape
        ), f"{valid_mask.shape=}, {cosine_map.shape=}"
        ...
        assert not torch.isnan(
            score
        ), f"{score=}, {cosine_map.min()=}, {cosine_map.max()=}, {torch.any(torch.isnan(cosine_map))=}, {torch.any(torch.isnan(torch.acos(cosine_map)))=}, {torch.any(torch.isnan(torch.rad2deg(torch.acos(cosine_map))))=}"
        # return
        return {'angle': score}
