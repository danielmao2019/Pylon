...
3. the core principles of lossless dtype casting:
 ...
 when torch does not support the target dtype, storage uses the smallest torch dtype whose set contains the target dtype's set, if one exists. ply u2 and numpy uint16 both go to int32, ply u4 and numpy uint32 both go to int64.
 ...
 and uses float64 storage if its actual values fit exactly. float32 and smaller dtypes are not tested.
 a ply u4 column loads as int64 because torch has no uint32,
               1. defensive programming: it asserts the cast is lossless.
 a target dtype ply does not have, such as int64, is never substituted for. save hard-asserts and the program aborts.
, so this is the user asking for int64 in a ply file, and it is the user's own doing.
...
3. color convention: conversion changes between color representations, such as 0 to 255 integer representation and 0 to 1 floating point representation.
 the dtype defines the convention: a float dtype means 0 to 1, an integer dtype means that dtype's own range. so a uint8 color is 0 to 255 and a uint16 color is 0 to 65535, which is what las stores.
...
1. what it is: meta data records what the source looked like, upon construction. it records the source of the data, wherever the data comes from: a load from disk, a construction from a torch tensor or a numpy array, or addition or deletion of fields. it records two things.
 ...
 the record holds uint32,
      2. a float128 source with no override records float128 in meta data
 and uint8 is what the record stores. no special treatment.
 the record keeps the dtype the file stores each coordinate column in
   2. layout:
...
7. load point cloud should take an optional arg to override the meta data, the same way save point cloud does, and it reaches both halves.
 ...
 it is the same control at per-field granularity
 the override covers the dtype it forced,
...
2. rgb enters and is held exactly as it arrived, like every other field.
 because fields keep their own names
   5. for both `__init__` and load point cloud,
      2. validation:
...
, so every caller passing dtype is updated.
 Select asserts it at the point of use instead
   2. point cloud I/O:
      1. load point cloud
...
, and a reader neither widens nor narrows what the file holds.
         2. a reader never widens a field it builds:
, so an f4 ply gives float32 xyz and an f8 ply gives float64 xyz.
      ...
      4. a las bit-packed field is an ordinary unsigned integer. laspy materializes it as uint8, so uint8 is what enters the obj
 ...
 save's color convention conversion is keyed on rgb and is the only such branch in the I/O layer.
4. rgb is the one field with convention conversion between color representations.
 save reads the convention off the field's current dtype and off the recorded dtype, and converts between the two.
...
- PointCloud requiring indices to be int64.
   3. the new API on the meta data override:
      1. this override replaces the existing dtype arg, which cast xyz alone.
      2. name_feat is removed.
 while its renaming of a named column to feat and its reshape to [N, 1] are dropped rather than replaced,
 and no caller outside a test passes it.
