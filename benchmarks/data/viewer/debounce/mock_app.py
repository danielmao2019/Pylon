...
from .mock_data import generate_synthetic_pcr_dataset, get_mock_datapoint
...
class MockViewerApp:
    ...
    def __init__(
        ...
        if use_debounce:
            self.update_datapoint_from_navigation = debounce(
                self._update_datapoint_from_navigation
            )
    ...
    def _update_datapoint_from_navigation(
        self, datapoint_idx: int, settings_3d: Dict, camera_state: Dict
    ) -> List[Dict]:
        ...
        datapoint = get_mock_datapoint(
            self.dataset, datapoint_idx, self.ui_state['selected_transforms']
        )
    ...
    def _update_index_from_buttons(
        self, prev_clicks: Optional[int], next_clicks: Optional[int], current_value: int
    ) -> List[int]:
    ...
    def _update_datapoint_from_transforms(
        self,
        ...
        selected_indices = [
            idx for values in transform_values for idx in values if values
        ]
    ...
    def _sync_camera_state(
        self, all_relayout_data: List[Dict], all_figures: List[Dict]
    ) -> tuple:
