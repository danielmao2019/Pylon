...
class CameraPoseSampler:
    ...
    def generate_poses_for_point_cloud(
        self, point_cloud: torch.Tensor, num_poses_per_distance: int = 3
    ) -> List[CameraPose]:
        ...
        print(
            f"    Distance groups: close={distance_groups['close']:.2f}, medium={distance_groups['medium']:.2f}, far={distance_groups['far']:.2f}"
        )
        ...
        for group_name, base_distance in distance_groups.items():
            for i in range(num_poses_per_distance):
                ...
                phi = self.rng.uniform(
                    np.pi / 6, np.pi / 3
                )  # Elevation angle (avoid top-down)
                ...
                side_offset = self.rng.uniform(
                    -diagonal_size * 0.2, diagonal_size * 0.2, 3
                )
