...
class ComprehensiveBenchmarkReportGenerator:
    ...
    def save_all_results_json(
        self,
        synthetic_results: Optional[Dict[str, List[BenchmarkStats]]] = None,
        ...
        if synthetic_results:
            ...
            for dataset_name, distance_results in synthetic_results.items():
                ...
                for distance_group, stats_list in distance_results.items():
                    serializable_synthetic[dataset_name][distance_group] = [
                        asdict(stats) for stats in stats_list
                    ]
        ...
        if real_data_results:
            ...
            for dataset_name, distance_results in real_data_results.items():
                ...
                for distance_group, stats_list in distance_results.items():
                    serializable_real[dataset_name][distance_group] = [
                        asdict(stats) for stats in stats_list
                    ]
    ...
    def create_speed_vs_distance_by_dataset_plots(
        self,
        results: Dict[str, Dict[str, List[BenchmarkStats]]],
        ...
        for dataset_name, distance_results in results.items():
            ...
            fig.suptitle(
                f'LOD Performance vs Camera Distance: {dataset_name.upper()} Dataset',
                fontsize=14,
                fontweight='bold',
            )
            ...
            for distance_group in ['close', 'medium', 'far']:
                ...
                if stats_list:
                    ...
                    point_reductions.append(
                        np.mean([s.point_reduction_pct for s in stats_list])
                    )
            ...
            for i, (bar1, bar2, speedup) in enumerate(
                zip(bars1, bars2, speedups, strict=True)
            ):
            ...
            total_samples = sum(
                len(stats_list) for stats_list in distance_results.values()
            )
            ...
            # Save plot
            plot_file = (
                self.output_dir / f"{data_type}_speed_vs_distance_{dataset_name}.png"
            )
    ...
    def create_speed_vs_dataset_by_distance_plots(
        self,
        results: Dict[str, Dict[str, List[BenchmarkStats]]],
        data_type: str = "synthetic",
    ):
        ...
        for distance_group, dataset_results in distance_organized.items():
            ...
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
            fig.suptitle(
                f'LOD Performance vs Dataset: {distance_group.upper()} Camera Distance',
                fontsize=14,
                fontweight='bold',
            )
            ...
            ax1.set_xticks(x)
            ax1.set_xticklabels(
                [name.capitalize() for name in dataset_names], rotation=45
            )
            ax1.legend()
            ax1.grid(True, alpha=0.3)
            ...
            avg_reduction = np.mean(point_reductions)
            total_samples = sum(
                len(stats_list) for stats_list in dataset_results.values()
            )
            ...
            plot_file = (
                self.output_dir
                / f"{data_type}_speed_vs_dataset_{distance_group}_distance.png"
            )
            plt.savefig(plot_file, dpi=300, bbox_inches='tight')
            plt.close()

            print(
                f"  📈 Saved {distance_group} distance speed vs dataset plot: {plot_file}"
            )

    def create_comprehensive_summary_report(
        self,
        synthetic_results: Optional[Dict[str, Dict[str, List[BenchmarkStats]]]] = None,
        real_data_results: Optional[Dict[str, Dict[str, List[BenchmarkStats]]]] = None,
    ):
        ...
        for data_type, results in [
            ...
            for dataset_name, distance_results in results.items():
                ...
                all_stats = [
                    s for stats_list in distance_results.values() for s in stats_list
                ]
                if all_stats:
                    ...
                    report_lines.append(
                        f"- **Average point reduction**: {np.mean(reductions):.1f}%"
                    )
                    ...
                    for distance_group in ['close', 'medium', 'far']:
                        stats_list = distance_results.get(distance_group, [])
                        if stats_list:
                            group_speedup = np.mean(
                                [s.speedup_ratio for s in stats_list]
                            )
                            group_reduction = np.mean(
                                [s.point_reduction_pct for s in stats_list]
                            )
                            report_lines.append(
                                f"  - **{distance_group.capitalize()} distance**: {group_speedup:.2f}x speedup, {group_reduction:.1f}% reduction"
                            )
            ...
            if overall_speedups:
                ...
                report_lines.append(
                    f"- **Overall average speedup**: {np.mean(overall_speedups):.2f}x"
                )
                report_lines.append(
                    f"- **Overall best speedup**: {max(overall_speedups):.2f}x"
                )
                report_lines.append(
                    f"- **Overall average point reduction**: {np.mean(overall_reductions):.1f}%"
                )
                report_lines.append(
                    f"- **Cases with >10% speedup**: {sum(1 for s in overall_speedups if s > 1.1)}/{len(overall_speedups)}"
                )
                report_lines.append("")
