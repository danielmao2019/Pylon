...
from .data_types import BenchmarkStats, PointCloudSample
from .streamers import RealDataPointCloudStreamer, SyntheticPointCloudStreamer
...
class SyntheticBenchmarkOrchestrator:
    ...
    def run_comprehensive_benchmark(
        self, num_configs: int = None
    ) -> Dict[str, Dict[str, List[BenchmarkStats]]]:
        ...
        for sample in self.point_cloud_streamer.stream_point_clouds(num_configs):
            ...
            camera_poses = self.camera_pose_sampler.generate_poses_for_point_cloud(
                sample.points, num_poses_per_distance=1
            )

            dataset_name = sample.metadata[
                'dataset'
            ]  # e.g., 'small', 'medium', 'large', 'xlarge'
            ...
            for camera_pose in camera_poses:
                ...
                results_by_dataset[dataset_name][camera_pose.distance_group].append(
                    stats
                )
    ...
    def run_quick_benchmark(self) -> List[BenchmarkStats]:
        ...
        for config in quick_configs:
            ...
            if far_poses:
                stats = self.benchmark_runner.benchmark_single_pose(
                    sample, far_poses[0]
                )
                results.append(stats)

                print(
                    f"    📈 {stats.speedup_ratio:.2f}x speedup, {stats.point_reduction_pct:.1f}% reduction"
                )
...
class RealDataBenchmarkOrchestrator:
    ...
    def run_real_data_benchmark(
        self, num_samples: int = 10, num_poses_per_distance: int = 3
    ) -> Dict[str, Dict[str, List[BenchmarkStats]]]:
    ...
    def _process_dataset_samples(
        self,
        dataset_name: str,
        samples: List[PointCloudSample],
        num_poses_per_distance: int,
    ) -> Dict[str, List[BenchmarkStats]]:
        ...
        for distance_group, stats_list in distance_results.items():
            if stats_list:
                ...
                print(
                    f"    📈 {distance_group.capitalize()}: {avg_speedup:.2f}x speedup, {avg_reduction:.1f}% reduction"
                )
